...
int main(int argc, char **argv){
  ...
  auto run_analysis=[noise,&generator,nombre_steps_mcmc,nombre_samples_collected,lb_t,ub_t,lb_hpars,ub_hpars,dim_theta,dim_hpars,X_init,COV_init](string foldname,VectorXd const &Xobs,VectorXd const &Yobs){
  ...
  auto run_analysis_MCMCadapt=[noise,&generator,nombre_steps_mcmc,nombre_samples_collected,lb_t,ub_t,lb_hpars,ub_hpars,dim_theta,dim_hpars,X_init,COV_init](string foldname,VectorXd const &Xobs,VectorXd const &Yobs){
    //fonction qui trouve un échantillon de la postérieure à partir du grid, des observations, et écrit tout dans un dossier.
    int nobs=Xobs.size();
    string endname=".gnu";
    int dim_x=1; // On considère que x vit seulement dans [0,1]. A normaliser si jamais on va en plus grande dimension.
  ...
  auto run_analysis_RD=[noise,&generator,nombre_steps_mcmc,nombre_samples_collected,lb_t,ub_t,lb_hpars,ub_hpars,dim_theta,dim_hpars,X_init,COV_init](string foldname,VectorXd const &Xobs,vector<VectorXd> const &Yobs_vect){
