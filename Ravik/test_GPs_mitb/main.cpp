// Dans ce fichier : je construis 3 DoE sur l'expérience 16. QMC, LHS, Prior sampling. Afficher les DoEs, et calculer la performance de l'un sur l'autre.
...
vector<VectorXd> drawPrior(default_random_engine &generator, int npts){
...
int main(int argc, char **argv){
  ...
  for (int i:cases){
    cout << "construction d'un DoE QMC..." << endl;

    int nmodel_evals=2000; //nombre d'évaluations du modèle pour faire le GP.
    //construction d'un grid en theta
    DoE doe_qmc(lb_t,ub_t,nmodel_evals,10);//doe halton
    DoE doe_qmcverif(lb_t,ub_t,nmodel_evals,10000);//doe halton
    DoE doe_LHS(lb_t,ub_t,nmodel_evals,generator);

    vector<VectorXd> thetas=doe_qmc.GetGrid();
    vector<VectorXd> thetas_qmc=doe_qmcverif.GetGrid();
    vector<VectorXd> thetas_lhs=doe_LHS.GetGrid();
    vector<VectorXd> thetas_prior=drawPrior(generator,nmodel_evals);
    //affichage du doe
    string fname="results/doeqmc.gnu";
    writeVector(fname,thetas);
    ...
    auto create_model_evals=[&lambda_model,&xobs](vector<VectorXd> & thetas){
      vector<VectorXd> modevals;
      for(int j=0;j<thetas.size();j++){
        VectorXd eval(xobs.size());
        for(int k=0;k<xobs.size();k++){
          eval(k)=lambda_model(xobs(k),thetas[j]);
        }
      modevals.push_back(eval);
    }
    return modevals;
    };
    ...
    vector<VectorXd> model_evals=create_model_evals(thetas);
    vector<VectorXd> model_evals_qmc=create_model_evals(thetas_qmc);
    vector<VectorXd> model_evals_lhs=create_model_evals(thetas_lhs);
    vector<VectorXd> model_evals_prior=create_model_evals(thetas_prior);
    // PCA
    int nmodes=2;
    ...
    writeVector(fgp,hp);
    cout << "performance QMC sur QMC" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_qmc,model_evals_qmc);
    cout << "performance QMC sur LHS" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_lhs,model_evals_lhs);
    cout << "performance QMC sur PRIOR" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_prior,model_evals_prior);
    v_gp.push_back(vgps);
    v_VP.push_back(VP);
  ...
  for (int i:cases){
    cout << "construction d'un DoE LHS..." << endl;

    int nmodel_evals=2000; //nombre d'évaluations du modèle pour faire le GP.
    //construction d'un grid en theta
    DoE doe_lhs(lb_t,ub_t,nmodel_evals,generator);//doe halton
    DoE doe_qmcverif(lb_t,ub_t,nmodel_evals,10000);//doe halton
    DoE doe_LHS(lb_t,ub_t,nmodel_evals,generator);

    vector<VectorXd> thetas=doe_lhs.GetGrid();
    vector<VectorXd> thetas_qmc=doe_qmcverif.GetGrid();
    vector<VectorXd> thetas_lhs=doe_LHS.GetGrid();
    vector<VectorXd> thetas_prior=drawPrior(generator,nmodel_evals);
    //affichage du doe
    string fname="results/doelhs.gnu";
    writeVector(fname,thetas);
    ...
    //écriture des hpars optimaux
    writeVector(fgp,hp);
    cout << "performance LHS sur QMC" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_qmc,model_evals_qmc);
    cout << "performance LHS sur LHS" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_lhs,model_evals_lhs);
    cout << "performance LHS sur PRIOR" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_prior,model_evals_prior);
    v_gp.push_back(vgps);
    v_VP.push_back(VP);
    v_Acoefs.push_back(Acoefs);
    ...
    for (int i:cases){
    cout << "construction d'un DoE Prior..." << endl;
    ...
    vector<VectorXd> thetas=drawPrior(generator,nmodel_evals);
    vector<VectorXd> thetas_qmc=doe_qmcverif.GetGrid();
    vector<VectorXd> thetas_lhs=doe_LHS.GetGrid();
    vector<VectorXd> thetas_prior=drawPrior(generator,nmodel_evals);
    //affichage du doe
    string fname="results/doeprior.gnu";
    writeVector(fname,thetas);
    ...
    //écriture des hpars optimaux
    writeVector(fgp,hp);
    cout << "performance PRIOR sur QMC" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_qmc,model_evals_qmc);
    cout << "performance PRIOR sur LHS" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_lhs,model_evals_lhs);
    cout << "performance PRIOR sur PRIOR" << endl;
    verif_gp(vgps,VP,Acoefs,featureMeans,thetas_prior,model_evals_prior);
    v_gp.push_back(vgps);
    v_VP.push_back(VP);
    v_Acoefs.push_back(Acoefs);
  ...
  for(int i=0;i<cases.size();i++){
    ...
    auto lambda_modeltrue=[&pFunc_init,&cases](VectorXd const &X, VectorXd const & theta) mutable{
      //renvoie toutes les prédictions du modèle aux points donnés par X.
...
for(int i=0;i<vsize;i++){
    ...
    // phase KOH separate
    auto hparskoh_separate=HparsKOH_separate(vDens,hpars_z_guess,240);
