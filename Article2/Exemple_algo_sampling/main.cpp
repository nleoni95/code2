...
int main(int argc, char **argv)
{
  ...
  //création des observations. tirées de la fonction exponentielle bruitée.
  double noise = 1e-2;
  int ndata = 8;
  ...
  for (int i = 0; i < ndata; i++)
  {
    double xmax=1;
    VectorXd x(1);
    x << xmax* double(i) / double(ndata-1);
  ...
  {
    VectorXd x(1);
    x << 0 + 1 * double(i) / double(samp_size-1);
    XPREDS[i] = x;
  }
  ...
  {
    //c'est ici qu'on transforme les theta.
    VectorXd lb_t(2); lb_t << -2,-2;
    VectorXd ub_t(2); ub_t << 2,2;
    VectorXd t=lb_t+(ub_t-lb_t).cwiseProduct(theta);
    VectorXd pred(X.size());
    for (int j = 0; j < X.size(); j++)
    {
      pred(j) = computer_model(X[j](0), t);
    }
    return pred;
  };
  ...
  MainDensity.SetOutputerr(false, 1e-4, 2);
  ...
  COV_init(0, 0) = pow(0.05, 2);
  COV_init(1, 1) = pow(0.05, 2);
  cout << "COV_init : " << endl
       << COV_init << endl;
