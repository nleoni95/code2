...
// calibration du diamètre uniquement
...
int main(int argc, char **argv){
...

  {
    ...
    auto in_bounds=[&MainDensity_diam](VectorXd const & X){
      if(X.size()==5){
        return MainDensity_diam.in_bounds_pars(X);
      }
      if(X.size()==8){
        return MainDensity_diam.in_bounds_pars(X.head(5)) && MainDensity_diam.in_bounds_hpars(X.tail(3));
      }
    ...
    hparskoh=MainDensity_diam.HparsKOH(hpars_z_guess_diam,200);
    cout << "hpars koh after optimisation : " << hparskoh.transpose() << endl;
      cout << endl << "Début KOH" << endl;
      VectorXd hparskoh_diam=hparskoh;
      cout << "hpars koh diam : " << hparskoh_diam.transpose() << endl;

      auto get_hpars_koh=[&hparskoh_diam](VectorXd const & X){
        auto p=make_pair(hparskoh_diam,hparskoh_diam);
        return p;
      };
      ...
      auto compute_score_koh=[&MainDensity_diam,&data_exp_alpha,&data_exp_diam,&LDLT2,&hparskoh_diam](pair<VectorXd,VectorXd> p, VectorXd const &X){
      ...
      //diagnostic
      MainDensity_diam.Autocor_diagnosis(nautocor,"results/diag/autocorkoh.gnu");
      ...
      
    {
      ...
      auto get_hpars_opti=[&DensOpt_diam](VectorXd const & X){
        VectorXd hparsopt_diam=DensOpt_diam.EvaluateHparOpt(X);
        auto p=make_pair(hparsopt_diam,hparsopt_diam);
        return p;
      };
        
            
      auto compute_score_opti=[&data_exp_alpha,&data_exp_diam,&DensOpt_diam](pair<VectorXd,VectorXd> p, VectorXd const &X){
      ...
      auto add_npoints=[&DensOpt_diam,&nombre_steps_mcmc,&Xinit,&COV_init,&compute_score_opti,&get_hpars_opti,&in_bounds,&Bounds_hpars_gp,&hpars_gp_guess](int npoints,default_random_engine &generator, int & npts_total, int nsamples_mcmc){
      ...
      for(int i=0;i<vhpars_opti.size();i++){
        ...
        //diagnostic
        DensOpt_diam.Autocor_diagnosis(nautocor,"results/diag/autocoropt+.gnu");
    ...
    
    {
      ...
      auto begin=chrono::steady_clock::now();
        ...
        auto compute_score_fb=[&MainDensity_diam](pair<VectorXd,VectorXd> p, VectorXd const &X){
        ...
        VectorXd Xinit_fb(8);
        Xinit_fb << Xinit(0),Xinit(1),Xinit(2),Xinit(3),Xinit(4),hpars_z_guess_diam(0),hpars_z_guess_diam(1),hpars_z_guess_diam(2);

        MatrixXd COV_init_fb=MatrixXd::Identity(8,8);
        COV_init_fb.topLeftCorner(5,5)=0.5*COV_init;

        COV_init_fb(5,5)*=pow(4e-4,2); //sedm avant 1e-7
        COV_init_fb(6,6)*=pow(4e-8,2); //sobs avant 1e-8
        COV_init_fb(7,7)*=pow(2e-3,2); //lcor avant 1e-3
      ...
      cout << "Temps pour BAYES : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << endl;
        ...
        //diagnostic
        MainDensity_diam.Autocor_diagnosis(nautocor,"results/diag/autocorfb.gnu");
