...
{
  /* This is the function you optimize for defining the GP */
  //auto pa=(pair<double *, GP*>*) data;
  pair<int*,GP*> *pa{static_cast<pair<int*,GP*>*>(data)};
  GP *proc = pa->second;       //Pointer to the GP
  (*pa->first)++;
  Eigen::VectorXd p(x.size()); //Parameters to be optimized
  for (int i = 0; i < (int)x.size(); i++)
    p(i) = x[i];                 //Setting the proposed value of the parameters
  double value = -1*proc->SetGP(p); //Evaluate the function. Attention ! 
...
void OptimizeGPBis(GP & gp, VectorXd &guess, VectorXd const & lb,VectorXd const & ub, double ftol, int &count){
	//version réécrite Nicolas
  auto p=make_pair(&count,&gp);
...
double Density::GetOutputerr(VectorXd const &hpars) const
{
  if (m_outputerr == -1)
  {
    //outputerr is learned. In log scale.
...
{
  //évaluation de la LDLT de Gamma. Si la taille change, on pourrait faire plusieurs gamma (plusieurs hpars.)
...
vector<vector<VectorXd>> DensityOpt::SampleHparsOpt(vector<VectorXd> const &thetas, int nsamples, default_random_engine & generator) const
{
  ...
  for(int i=0;i<m_hGPs.size();i++){
    MatrixXd M=m_hGPs[i].SampleGPDirect(thetas,nsamples,generator); // dimension : (thetas.size(),nsamples)
...
void DensityOpt::OptimizeHGPs(Eigen::MatrixXd Bounds_hpars_GPs, vector<VectorXd> Hpars_guess_GPs,double ftol,VectorXi &count)
{
  for (int i = 0; i < m_hGPs.size(); i++)
  {
    ...
    auto begin = chrono::steady_clock::now();
    OptimizeGPBis(m_hGPs[i],Hpars_guess_GPs[i], Bounds_hpars_GPs.row(0),Bounds_hpars_GPs.row(1),ftol,count(i));
    auto end = chrono::steady_clock::now();
