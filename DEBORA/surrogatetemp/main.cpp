...
int main(int argc, char **argv){
  ...
  // pour la MCMC
  MatrixXd COV_init=pow(0.09,2)*MatrixXd::Identity(5,5);
  MatrixXd COV_init_bayes=MatrixXd::Identity(8,8);
  COV_init_bayes.topLeftCorner(5,5)=COV_init;
  COV_init_bayes(5,5)*=pow(5e-5,2); //sedm avant 1e-7
  COV_init_bayes(6,6)*=pow(9e-8,2); //sobs ava,t 1e-8
  COV_init_bayes(7,7)*=pow(2e-3,2); //lcor avant 1e-3
 ...
 /*
  ...
  cout << "evidence bayes : " << MainDensity.Evidence(30);
  ...
  */
  ...
  DensOpt.Test_hGPs();
  //pas d'optimisation des hpars gp pour gagner du temps.
  //DensOpt.ReadhGPs("results/save/hGPsdiammat5.gnu");
  ...
  cout << "evidence opti : " << DensOpt.Evidence(30);
