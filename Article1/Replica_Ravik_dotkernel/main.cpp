...
int main(int argc, char **argv)
{
  ...
  ub_hpars(0) = 0.8;
  ub_hpars(1) = 1e6;
  ub_hpars(2) = 30; //ub_hpars(2)=1e6; inputerr en position 0.
  ...
  //vector<int> cases = {3, 4, 5, 6, 8, 15, 16, 20, 21}; // good cases
  ...
  for (int i = 0; i < cases.size(); i++)
  {
    ...
    MainDensity.SetOutputerr(false, 10, 1); //outputerr à 10 pour + de stabilité.
    ...
    MainDensity.SetInputerr(true, 999, 0, derivatives.first, derivatives.second); //appris.
  ...
  // calibration BAYES
  {
    ...
    auto compute_score_fb = [&vsize, &lb_hpars, &vDens](vector<VectorXd> p, VectorXd const &X)
    {
      ...
      for (int i = 0; i < vsize; i++)
      {
        double d = vDens[i].loglikelihood_theta(theta, hp[i]);
        res += d;
      }
      res += logprior_pars(theta);
      return res;
