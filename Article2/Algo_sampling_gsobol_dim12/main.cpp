...
double logprior_hpars(VectorXd const &hpars)
{
  ...
  //on met inverse gamma aussi pour sigma.
  double asigma =10;
  double bsigma =1;
...
int main(int argc, char **argv)
{
  ...
  double ftol_rel_gp_llsurrogate = 0.5;
  ...
  // voir combien de temps prennent les optimisations FMP avec 50 observations.
  {
    ftol_rel_fmp = 1e-5;
    hpars_post.clear();
    scores_post.clear();
    ...
    for (int i = 0; i < thetas_post.size(); i++)
    {
      scores_post.push_back(Dopt.loglikelihood_theta(thetas_post[i], hpars_post[i]));
    }
    double time = chrono::duration_cast<chrono::milliseconds>(end - begin).count();
    double mean = std::accumulate(scores_post.begin(), scores_post.end(), 0.0) / scores_post.size();
    cout << "ftol rel = " << ftol_rel_fmp << endl;
    cout << "mean scores : " << mean << endl;
    cout << "time per opt : " << time/thetas_post.size() << endl;
  }
...
exit(0);
  ...
  /*
  {
    ...
    Dopt.OptimizeHGPs(Bounds_hpars_HGPs, hpars_opt_hgps, 5);
  ...
  */
  ...
  /*20 répétitions de hparssurrogate avec adaptive sampling, poids calculés par variance de la fct de vraisemblance. dossier as50local*/
  {
    int nrepet = 20;
    for (int k = 0; k < nrepet; k++)
    {
      string foldname = "as50_local/s" + to_string(k);
      // paramètres de l'algorithme
      int ndraws = 100; // nombre de répétitions pour estimer la variance lors du calcul des poids.
      int npts_init = 20;
      int nmax = 800;
      int npts_per_iter = 50;
      int nsteps_mcmc = 12.5e4;
      ...
      auto add_points = [&Dopt, nsteps_mcmc, &X_init_mcmc, &COV_init, &generator, hpars_z_guess, &thetas_training, &hpars_training, ftol_rel_fmp, ndraws, nsamples_mcmc, npts_per_iter](string foldname, VectorXd &times)
      {
        ...
        Nodups(candidate_thetas);
        cout << "number of candidate thetas after removing duplicates: " << candidate_thetas.size() << endl;
        auto t2 = chrono::steady_clock::now();
        times(0) = chrono::duration_cast<chrono::milliseconds>(t2 - t1).count();
        vector<VectorXd> selected_thetas(npts_per_iter);
        vector<double> weights(candidate_thetas.size());
  ...
  /*10 répétitions de hparssurrogate avec adaptive sampling, poids calculés par variance de la fct de vraisemblance. dossier as10 local*/
  {
    int nrepet = 20;
    for (int k = 0; k < nrepet; k++)
    {
      string foldname = "as10_local/s" + to_string(k);
      // paramètres de l'algorithme
      int ndraws = 100; // nombre de répétitions pour estimer la variance lors du calcul des poids.
      int npts_init = 20;
      int nmax = 800;
      int npts_per_iter = 10;
      int nsteps_mcmc = 2.5e4;
  ...
  /*10 répétitions de hparssurrogate avec LHS. Calibration et slice avec la dernière loop. dossier lhs*/
  {
    int nrepet = 20;
    for (int k = 0; k < nrepet; k++)
    {
  ...
  /*étude du temps de calcul minimal pour une optimisation FMP*/
  {
    ...
    // on suppose que seul le nombre de points compte (et non leur localisation)
    DensityOpt Dopt(MainDensity);
