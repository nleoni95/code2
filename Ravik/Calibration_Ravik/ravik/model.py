# -*- coding: utf-8 -*-
# on reproduit la fonction de Ravik (annexe B, page 123 de sa thèse)
...
chosen_case=0 #variable globale qui permet de savoir quel cas a été choisi. changée par la fonction "initialize_case"

steamTable = XSteam(XSteam.UNIT_SYSTEM_MKS)   # m/kg/sec/°C/bar/W

ParamsRavik=np.array([40,18.9E-6,0.27,0.75,-0.3,-0.26,6.1E-3,0.6317,0.1237,-0.373]) ## paramètres nominaux pour le modèle de Ravik. correspondent à différents coefficients dans les corrélations. Pour voir exactement à quoi ils correspondent, cf. la fonction heat_partitioning (argument param)

acceptable_cases={3,4,5,6,14,15,16,18,20,21,22,23} #cas expérimentaux pour lesquels on dispose d'observations

#conditions expérimentales. dans l'ordre : numéro du cas, pression, vitesse, température de subcooling, diamètre hydraulique.
exp_conditions=[
    [3,2,3.66,27.8,19.56],
    [4,2,0.3,27.8,19.56],
    [5,2,1.22,27.8,19.56],
...
def heat_partitioning(p,Twall,Tbulk,vel,Dh,param):
    # modèle de Ravik. on n'appelle pas directement cette fonction, seulement à travers "chaleur".
...
def chaleur(p,DTsup,DTsub,vel,Dh,param):
    #on n'appelle pas directement cette fonction, seulement à travers "run_model"
...
def run_model(DTsup,param):
    #fonction à lancer pour faire tourner le code Ravik avec les paramètres param.
    #on récupère les conditions expérimentales du cas chosen_case
    exp_cond=pd.DataFrame(data=exp_case(),columns=['case nr','p','v','DTsub','diam'])
...
def initialize_case(case):
    ...
    #assigne à la variable globale "chosen_case" le numéro du cas expérimental souhaité.
    global chosen_case
...
def exp_datab():
    #renvoie les observations du cas chosen_case.
    casestr=str(chosen_case)
    kennel=np.loadtxt('data/Kennel'+casestr+'.csv',delimiter=",")
    return kennel.tolist()

def exp_case():
    #renvoie les conditions expérimentales du cas chosen_case
    return (exp_conditions.loc[exp_conditions['case nr']==chosen_case]).to_numpy()

def plot_model():
    #boucle pour faire tourner le modèle et afficher les résultats
    X=np.linspace(0.001,20,100) #changer les bornes si le cas expérimental possède des obesrvations jusqu'à 25K de DT par exemple.
    Y1=np.empty([100])
    Y2=np.empty([100])
    Y3=np.empty([100])
    params=ParamsRavik
    for i in np.arange(100):
        Y1[i]=run_model(X[i],ParamsRavik)[0]
        Y2[i]=run_model(X[i],ParamsRavik)[1]
        Y3[i]=run_model(X[i],ParamsRavik)[2]

    plt.figure(figsize=[6,6])
    ax=plt.subplot(111)
    plt.plot(X,Y1+Y2+Y3,label='whf',color='orange')

    plt.plot(X,Y1,label='phifc',color='blue')
    plt.plot(X,Y2,label='phisc',color='green')
    plt.plot(X,Y3,label='phiie',color='brown')
    obs=np.array(exp_datab())
    print(obs)
    plt.scatter(obs[:,0],obs[:,1],label='observations',color='tab:red')
    ax.set_xlim(0,20)
    ax.set_ylim(0,1.2e6)
...
###début du script
initialize_case(20) #on fait tourner le cas Kennel 20
plot_model()
