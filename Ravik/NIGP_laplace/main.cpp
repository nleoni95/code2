...
// Dans ce fichier : on va échantillonner chaque densité séparément, puis construire une approx de laplace pour chacune, et tenter d'éch. la post globale en tant que produit de gaussiennes. J'espère que ça va marcher. Déjà si ça mixe à 150 (grand max) et qu'on veut 500 points, il faut 75000 points de la MCMC pour la précision. Ah mais en fait, on va simplement trouver les maxima des densités ? ça serait plus rapide en effet. Mais c'est bien de les sampler pour s'assurer que la Laplace est bonne. peut-être dans une seconde étape.
...
/*fonctions liées aux hessiennes*/

MatrixXd compute_Hessian(vector<double> const &xmax, nlopt::vfunc f,void *data_ptr){
  //dérivées de -log p hein.
  //avec approximations centrées https://www.sfu.ca/sasdoc/sashtml/iml/chap11/sect8.htm
  vector<double> grad;
  f(xmax,grad,data_ptr);
  double distance=0.05; //même distance dans les trois dimensions.

  auto get_point=[xmax,distance](VectorXd const & index){
    //renvoie un point décalé de xmax, en fonction de l'argument index dont les coordonnées valent -1, 0 ou 1. ou 2 en fait.
    vector<double> xres(xmax.size());
    for(int i=0;i<xmax.size();i++){
      xres[i]=xmax[i]+index(i)*distance;
    }
    return xres;
  };

  MatrixXd Hess(xmax.size(),xmax.size());
  for(int i=0;i<xmax.size();i++){
    for(int j=i;j<xmax.size();j++){    
      if(j>i){  
        VectorXd indpp=VectorXd::Zero(xmax.size()); indpp(i)+=1; indpp(j)+=1;
        VectorXd indpm=VectorXd::Zero(xmax.size()); indpm(i)+=1; indpm(j)-=1;
        VectorXd indmp=VectorXd::Zero(xmax.size()); indmp(i)-=1; indmp(j)+=1;
        VectorXd indmm=VectorXd::Zero(xmax.size()); indmm(i)-=1; indmm(j)-=1;
        double fpp=f(get_point(indpp),grad,data_ptr);
        double fpm=f(get_point(indpm),grad,data_ptr);
        double fmp=f(get_point(indmp),grad,data_ptr);
        double fmm=f(get_point(indmm),grad,data_ptr);
        Hess(i,j)=-(fpp+fmm-fpm-fmp)/(4*pow(distance,2));
        Hess(j,i)=Hess(i,j);
      }
      else if(i==j){
        VectorXd indpp=VectorXd::Zero(xmax.size()); indpp(i)+=1; indpp(i)+=1;
        VectorXd indp=VectorXd::Zero(xmax.size()); indp(i)+=1;
        VectorXd indm=VectorXd::Zero(xmax.size()); indm(i)-=1; 
        VectorXd indmm=VectorXd::Zero(xmax.size()); indmm(i)-=1; indmm(i)-=1;
        VectorXd indz=VectorXd::Zero(xmax.size());
        double fpp=f(get_point(indpp),grad,data_ptr);
        double fp=f(get_point(indp),grad,data_ptr);
        double fm=f(get_point(indm),grad,data_ptr);
        double fmm=f(get_point(indmm),grad,data_ptr);
        double fz=f(get_point(indz),grad,data_ptr);
        Hess(i,i)=-(-fpp-fmm+16*fp+16*fm-30*fz)/(12*pow(distance,2));
...
double evaluate_logmvn(VectorXd const & X,VectorXd const &mode, MatrixXd const & hess){
  //attention la hessienne est l'inverse de la matrice de covariance.
  int d=mode.size();
  LDLT<MatrixXd> ldlt(hess);
  VectorXd Alpha=hess*(X-mode);
  cout << "vps : " << ldlt.vectorD().transpose() << endl;
  cout << -0.5*(X-mode).dot(Alpha) << endl;
  cout << 0.5*(ldlt.vectorD().array().abs().log()).sum() << endl;
  double ll=-0.5*(X-mode).dot(Alpha)+0.5*(ldlt.vectorD().array().log()).sum()-0.5*d*log(2*3.1415);
  return ll;
}

vector<VectorXd> drawsample_mvn(VectorXd const &mode, MatrixXd const & hess, int nsamples,default_random_engine &generator){
  //on donne en argument la hessienne, qui est l'inverse de la cov matrix.
  vector<VectorXd> samples(nsamples);
  std::normal_distribution<double> distN(0,1);
  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> DEC(hess);
  Eigen::VectorXd D = DEC.eigenvalues();
	for(unsigned i=0; i<D.rows(); i++) D(i) = sqrt(fabs(D(i)));
  int d=mode.size();
  for(int i=0;i<nsamples;i++){
    VectorXd N(d); for(int j=0;j<d;j++){N(j)=distN(generator)*D(j);}
    VectorXd V=mode+DEC.eigenvectors()*N;
    samples[i]=V;
  }
  return samples;
}
...
int main(int argc, char **argv){
  ...
  MatrixXd COV_init=pow(0.005,2)*MatrixXd::Identity(dim_theta,dim_theta);
  ...
  vector<int> cases={3,4,5,6,14,15,16,18,20,21,22,23};
  ...
  //phase approx de Laplace: trouver le maximum d'une densité, puis estimer la hessienne à ce point. Ecrire le tout. Ou alors franchement faire un KDE semble un peu mieux ? surtout pour l'évaluation ? commençons par le plus facile. 
  {
    //la fonction à optimiser est définie à l'extérieur.
    for(int i=0;i<vsize;i++){
      ...
      VectorXd xmax(xguess.size()); for(int j=0;j<xmax.size();j++){xmax(j)=xguess[j];} 
      cout << "xmax pour expérience " << cases[i] << " : " << xmax.transpose() << endl;
      //calcul de la hessienne
      MatrixXd Hess=compute_Hessian(xguess,optfunc_opti,ptr);
      cout << "hessienne : " << endl << Hess << endl;
      VectorXd xtest=0.5*VectorXd::Ones(3);
      cout << "values : " << evaluate_logmvn(xmax,xmax,Hess) << " " << evaluate_logmvn(xtest,xmax,Hess) << endl;

      //écrire nos trouvailles dans un fichier.
      string fname="results/laplace"+to_string(cases[i])+".gnu";
