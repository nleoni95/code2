...
MatrixXd Density::VarParamUncertainty(vector<VectorXd> const &vectorx) const {
...
VectorXd DensityKOH::MeanPred(vector<VectorXd> &vectorx) const{
  //ici : on moyenne sur les theta. Alors que pour opti on tire un theta et on tire une réalisation. C'est plus accurate.
...
VectorXd DensityKOH::MeanPredCondTheta(std::vector<Eigen::VectorXd> &vectorx,int t) const{
  ...
  for (int i=0;i<vectorx.size();i++){pmean(i)=m_priormean(vectorx[i],Hparscourant)+(m_model_evals_predictions)[i](t);}
...
VectorXd DensityKOH::DrawSample(vector<VectorXd> &vectorx,default_random_engine &generator) const{
  ...
  VectorXd avg=MeanPredCondTheta(vectorx,indcour); //renvoie les prédictions moyennes
  MatrixXd COV=VarPred(vectorx); //calcul de la matrice de covariance de prédiction.
  VectorXd Sample=DrawMVN(avg,COV,generator);
  return Sample;
}

double DensityKOH::VarPred(VectorXd &xstar) const{ //new
  ...
  vector<VectorXd> v(1);
  v[0]=xstar;
  return VarPred(v)(0,0);
}

MatrixXd DensityKOH::VarPred(vector<VectorXd> &vectorx) const{ //new
  //renvoie la variance de prédiction prédiction du modèle en xstar. 
  //phase de calcul de la matrice de cov. et du vecteur associé.
  return VarPredCondTheta(vectorx,0); //car varpred ne dépend pas de theta.
}

MatrixXd DensityKOH::VarPredCondTheta(std::vector<Eigen::VectorXd> &vectorx,int t) const{ //new
  if(t>m_npts){cout << "erreur" << endl;}
  VectorXd Hparscourant=m_hpars; //valeur des hpars
...
void DensityKOH::WritePredictions(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de la prédiction moyenne et de l'incertitude associée, obtenue par méthode KOH. On fait 3 écart-type.
  VectorXd Mean=MeanPred(vectorx);//prédiction moyenne
  MatrixXd EDM=VarPred(vectorx); //variance de prédiction de z : E[c* -c*K-1c*] : "erreur de prédiction due à l'edm"
  MatrixXd Varf=VarParamUncertainty(vectorx); //variance de f_theta+z_theta : "erreur de prédiction due à l'incertitude paramétrique"
  ...
  {
    Contribf(i)=3*Varf.diagonal()(i)/(sqrt(Varf.diagonal()(i)+EDM.diagonal()(i)));
    ContribEDM(i)=3*EDM.diagonal()(i)/(sqrt(Varf.diagonal()(i)+EDM.diagonal()(i)));
  ...
  for (int i=0;i<Mean.size();i++)
  {
    fprintf(out, "%e %e %e %e\n",vectorx[i](0),Mean(i),Contribf(i),ContribEDM(i));
  }
  fclose(out);
}
...
void DensityOpt::WriteMeanPreds(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de toutes les prédictions moyennes
  FILE* out=fopen(file_name,"w");
  vector<VectorXd> Preds;
  for(int t=0;t<m_npts;t++){
    if(m_filtre[t]){
      VectorXd Pred=MeanPredCondTheta(vectorx,t);
      Preds.push_back(Pred);
    }
  }
  //phase de test : pondération par les probabilités.
  for (int i=0;i<vectorx.size();i++){
    fprintf(out,"%e ",vectorx[i](0));
...
void DensityKOH::WriteMeanPreds(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de toutes les prédictions moyennes
  FILE* out=fopen(file_name,"w");
  vector<VectorXd> Preds;
  for(int t=0;t<m_npts;t++){
...
void DensityOpt::WritePredictions(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de la prédiction moyenne et de l'incertitude associée, obtenue par méthode KOH. On fait 3 écart-type.
  VectorXd Mean=MeanPred(vectorx);//prédiction moyenne
  MatrixXd EDM=VarPred(vectorx); //variance de prédiction de z : E[c* -c*K-1c*] : "erreur de prédiction due à l'edm"
  MatrixXd Varf=VarParamUncertaintyTotal(vectorx); //variance de f_theta+z_theta : "erreur de prédiction due à l'incertitude paramétrique"
...
VectorXd DensityOpt::MeanPredCondTheta(std::vector<Eigen::VectorXd> const &vectorx,int t) const{
  //renvoie f_theta+z_theta mean.
  if(t>m_npts){cout << "erreur" << endl;}
...
double DensityOpt::VarPred(VectorXd &xstar) const{
  ...
  vector<VectorXd> X(1);
  X[0]=xstar;
  return VarPred(X)(0,0);
}

MatrixXd DensityOpt::VarPred(vector<VectorXd> &vectorx) const{
  //renvoie la variance de prédiction prédiction du modèle en xstar. 
  //phase de calcul de la matrice de cov. et du vecteur associé.
  MatrixXd varpred=MatrixXd::Zero(vectorx.size(),vectorx.size());
  for(int t=0;t<m_npts;t++){
    if(m_filtre[t]){
...
MatrixXd DensityOpt::VarParamUncertaintyTotal(vector<VectorXd> const &vectorx) const {
  //Calcul de Var(f_theta+z_theta) aux points du vectorx. Puisque cette variance est indépendante entre les points, on renvoie une matrice diagonale.
  //first : récupérer le vecteur d'évaluations du modèle aux points d'intérêt.
  int xsize=vectorx.size();
  //un premier calcul pour savoir combien de thetas ont une probabilité non nulle
  int tsize(0);
  for (int i=0;i<m_npts;i++){if(m_filtre[i]){++tsize;}}
  //on fait les évaluations du modèle
  vector<VectorXd> model_evals(tsize);
  VectorXd weights(tsize);
  int c(0);
  for (int i=0;i<m_npts;i++){if(m_filtre[i]){weights(c)=m_values[i]*m_Grid.GetWeights()[i];++c;}}
  if(weights.sum()!=1){cout << weights.sum() << endl;}
  c=0;
  for(int t=0;t<m_npts;t++){
    VectorXd X(xsize);
    if(m_filtre[t]){
      X=MeanPredCondTheta(vectorx,t);
      model_evals[c]=X;
      ++c;
    }
  }
  ...
  for (int i=0;i<xsize;i++){
    ...
    double esp(0);
    for (int j=0;j<tsize;j++){
      esp+=weights(j)*model_evals[j](i);
      var+=weights(j)*pow(model_evals[j](i),2);
    }
    COV(i,i)=var-pow(esp,2);
  }
  return COV;
...
MatrixXd DensityOpt::VarModelError(vector<VectorXd> &vectorx) const {
  //renvoie la variance de prédiction due à l'erreur de modèle.
  return VarPred(vectorx); //c'est déjà calculé par la fct VarPred
}
...
{
  //variance prédictive. Mais ça ne renvoie que Esp(var, il manque var(Esp))...
...
MatrixXd MCMC::VarParamUncertainty(vector<VectorXd> &vectorx) const
{
  //Calcul de Var(f_theta+z_theta) aux points du vectorx. Puisque cette variance est indépendante entre les points, on renvoie une matrice diagonale.
  //first : récupérer le vecteur d'évaluations du modèle aux points d'intérêt.
  int xsize=vectorx.size();
  //on fait les évaluations du modèle
  vector<VectorXd> model_evals(m_selected_samples.size()); //attention c'est l'inverse des autres implémentations où ce vecteur est de taille xsize
  for (int i=0;i<m_selected_samples.size();i++){
    model_evals[i]=MeanPredCondX(vectorx,m_selected_samples[i]); //VectorXd de taille xsize
  }
  //ok j'ai mon vecteur d'évaluations du modèle. Maintenant il faut calculer sa variance.
  MatrixXd COV=MatrixXd::Zero(xsize,xsize);
  for (int i=0;i<xsize;i++){
    double var(0);
    double esp(0);
    for (int j=0;j<m_selected_samples.size();j++){
      esp+=model_evals[j](i);
      var+=pow(model_evals[j](i),2);
    }
    esp/=double(m_selected_samples.size());
    var/=double(m_selected_samples.size());
    COV(i,i)=var-pow(esp,2);
  }
  return COV;
}

void MCMC::WritePredictions(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de la prédiction moyenne et de l'incertitude associée, obtenue par méthode KOH. On fait 3 écart-type.
  VectorXd Mean=MeanPred(vectorx);//prédiction moyenne
  MatrixXd EDM=VarPred(vectorx); //variance de prédiction de z : E[c* -c*K-1c*] : "erreur de prédiction due à l'edm"
  MatrixXd Varf=VarParamUncertainty(vectorx); //variance de f_theta+z_theta : "erreur de prédiction due à l'incertitude paramétrique"
