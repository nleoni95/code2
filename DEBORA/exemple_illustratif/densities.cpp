...
double Density::loglikelihood_obs_i(Eigen::VectorXd const &obsv, int i,Eigen::VectorXd const &hpars)const{
  ...
  double obs=obsv(i);
  double var=m_Kernel(m_Xprofile_converted[i],m_Xprofile_converted[i],hpars)+pow(m_noise,2);
...
VectorXd Density::map()const{
  //renvoie l'adresse du MAP parmi les samples contenus dans m_sample.
  //la fonction de référence est loglikelihood seule.
  return m_samples[max_ll()];
}

double Density::Var()const{
  //renvoie la variance de la post theta.
  double mean=0;
  double sm=0;
  int n=m_samples.size();
  for(int i=0;i<n;i++){
    mean+=m_samples[i](0);
    sm+=pow(m_samples[i](0),2);
  }
  double var=sm/n-pow(mean/n,2);
  return var;
}
...
int Density::WritePredictions(VectorXd const & X, string const & filename) const{
  //de plus, tirage de multiples échantillons de z.
  MatrixXd fvalues(X.size(),m_samples.size());
  for(int i=0;i<m_samples.size();i++){
    //VectorXd v=varZCondTheta(X,m_samples[i],m_hparsofsamples[i]).diagonal();
    fvalues.col(i)=m_model(X,m_samples[i])+meanZCondTheta(X,m_samples[i],m_hparsofsamples[i]);
    //fvalues.col(3*i+1)=fvalues.col(3*i).array()+3*v.array().sqrt();
    //fvalues.col(3*i+2)=fvalues.col(3*i).array()-3*v.array().sqrt();
  }
  //vecteurs des quantiles
  VectorXd quant2p5(X.size()); //95pct de la masse
  VectorXd quant97p5(X.size());
  VectorXd quant25(X.size()); //50 pct de la masse
  VectorXd quant75(X.size());
  for(int i=0;i<X.size();i++){
  ...
  for(int i=0;i<indexes.size();i++){
    if(m_data_exp[0].Value()(i)>=quant2p5(indexes[i])-m_noise && m_data_exp[0].Value()(i)<=quant97p5(indexes[i])+m_noise){
      counter++;
    }
    else{
      cout << "data nr " <<indexes[i] << " not in ci. data : " << m_data_exp[0].Value()(i) << ", low : " << quant2p5(indexes[i]) << ", sup : " << quant97p5(indexes[i]) << endl;
    }
  }
  cout << "nombre obs ds intervalle f+z : " << counter << endl;
  return counter;
}
...
int Density::WritePredictionsF(VectorXd const & X, string const & filename) const{
  ...
  //vecteurs des quantiles
  int ind_max=max_ll();
  cout << "theta MAP : " << m_samples[ind_max].transpose() << endl;
  ...
  //compte le nombre d'observations dans les intervalles de confiance à 95 pct de f.
  int counter=0;
  vector<int> indexes={10,20,30,40,50,60,70};
  for(int i=0;i<indexes.size();i++){
    if(m_data_exp[0].Value()(i)>=quant2p5(indexes[i])-m_noise && m_data_exp[0].Value()(i)<=quant97p5(indexes[i])+m_noise){
  ...
  cout << "nombre obs ds intervalle : " << counter << endl;
...
void DensityOpt::Compute_optimal_hpars(double max_time){
  ...
  cout << "stds  : " << sqrt(Var(0,0)) << " " << sqrt(Var(1,1)) << " " << endl;
