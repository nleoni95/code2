...
void WriteVector(vector<DATA> const &v, string const &filename)
{
  //write a vector to a file
  ofstream ofile(filename);
  int size = v[0].GetX().size();
  for (int i = 0; i < v.size(); i++)
  {
    for (int j = 0; j < size; j++)
    {
    ...
    ofile << v[i].Value() << endl;
  }
  ofile.close();
}

void WriteVectors(vector<VectorXd> const &v1, vector<double> const &v2, string const &filename)
{
...
void WriteVectors(vector<VectorXd> const &v1, vector<VectorXd> const &v2, vector<double> const &v3, string const &filename)
{
  //write two vectors of same size to a file
  if (!(v1.size() == v2.size()) || !(v1.size() == v3.size()))
  {
...
double optroutine_local(nlopt::vfunc optfunc, void *data_ptr, VectorXd &X, VectorXd const &lb_hpars, VectorXd const &ub_hpars, double max_time)
{
  //routine d'optimisation locale
  vector<double> x = VXDtoV(X);
...
void OptimizeGPBis_local(GP & gp, VectorXd &guess, VectorXd const & lb,VectorXd const & ub, double time){
	//version réécrite Nicolas
	// std::cout << "Optimize Gaussian process for " << np << " hyperparameters\n";
  optroutine_local(myoptfunc_gp,&gp,guess,lb,ub,time);
...
{
  //tirage de multiples réalisations des hyperparamètres optimaux, pour plusieurs valeurs de paramètres.
  //on ne sample pas sur plus de 200 points. Sinon, on utilise la GPLarge.
  int nthetas=thetas.size();
  int ns=min(nthetas,200);
  vector<vector<VectorXd>> res;
  vector<MatrixXd> res_malformatte; //utiliser ce format pour utiliser la fonction SampleGPDirect d'Olivier. On remet dans le bon format ensuite
  for(int i=0;i<m_hGPs.size();i++){
    MatrixXd M=m_hGPs[i].SampleGPLarge(thetas,ns,nsamples,generator); // dimension : (thetas.size(),nsamples)
    //scaling
    //M=M*m_scales_hGPs(i)+m_means_hGPs(i)*MatrixXd::Ones(M.rows(),M.cols());
    res_malformatte.push_back(M);
  }
  //mise dans le bon format
...
vector<MatrixXd> DensityOpt::SampleHparsOpt2(vector<VectorXd> const &thetas, int nsamples, default_random_engine & generator) const
{
  //tirage de multiples réalisations des hyperparamètres optimaux, pour plusieurs valeurs de paramètres.
  int nthetas=thetas.size();
  int ns=min(nthetas,100);
  vector<vector<VectorXd>> res;
  vector<MatrixXd> res_malformatte; //utiliser ce format pour utiliser la fonction SampleGPDirect d'Olivier. On remet dans le bon format ensuite
  for(int i=0;i<m_hGPs.size();i++){
    MatrixXd M=m_hGPs[i].SampleGPLarge(thetas,ns,nsamples,generator); // dimension 
    //scaling
    //M=M*m_scales_hGPs(i)+m_means_hGPs(i)*MatrixXd::Ones(M.rows(),M.cols());
    res_malformatte.push_back(M);
  }
  return res_malformatte;
}
...
void DensityOpt::OptimizeHGPs_local(Eigen::MatrixXd Bounds_hpars_GPs, vector<VectorXd> Hpars_guess_GPs,double time)
{
  for (int i = 0; i < m_hGPs.size(); i++)
  {
    m_hGPs[i].SetGP(Hpars_guess_GPs[i]);
    cout << "local optimization of HGP number " << i << endl;
    //cout << "guess : " << Hpars_guess_GPs.transpose() << endl;
    auto begin = chrono::steady_clock::now();
    OptimizeGPBis_local(m_hGPs[i],Hpars_guess_GPs[i], Bounds_hpars_GPs.row(0),Bounds_hpars_GPs.row(1),time);
    auto end = chrono::steady_clock::now();
    cout << "optimisation over in " << chrono::duration_cast<chrono::seconds>(end - begin).count() << " seconds." << endl;
    cout << "result : " << m_hGPs[i].GetPar().transpose() << endl;
  }
