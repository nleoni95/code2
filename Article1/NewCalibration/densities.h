...
class Density
{
    ...
    //fonctions avec l'intertitude sur l'input

    Eigen::MatrixXd IncX(std::vector<Eigen::VectorXd> const & locs) const;
    Eigen::VectorXd modderiv(std::vector<Eigen::VectorXd> const & locs, Eigen::VectorXd const &theta,Eigen::VectorXd const &hpar) const;
    double loglikelihood_theta_incx(Eigen::VectorXd const &theta, Eigen::VectorXd const &hpar) const;
    void WriteObsWithUncertainty(std::string const filename,double sobs) const;
    ...
    double optroutine(nlopt::vfunc optfunc,void *data_ptr, Eigen::VectorXd &x, Eigen::VectorXd const & lb_hpars, Eigen::VectorXd const & ub_hpars, double max_time) const;
    ...
    double TestHparsKOH(Eigen::VectorXd const & hpars_guess) const;
    ...
    Eigen::VectorXd meanZCondTheta_nox(Eigen::VectorXd const & X,Eigen::VectorXd const & theta, Eigen::VectorXd const &hpars_z) const;
    ...
    void WriteSamplesFandZ(Eigen::VectorXd const &X,std::string const & filenameF, std::string const & filenameZ) const;
    ...
    std::function<Eigen::VectorXd(Eigen::VectorXd const &,Eigen::VectorXd const &)> GetModel() const{ return m_model;}

    protected:

    Eigen::MatrixXd m_incx_obs; //matrice des incertitudes aux poitns d'observation.
