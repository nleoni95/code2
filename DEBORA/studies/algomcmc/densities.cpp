...
DoE::DoE(VectorXd const &lb, VectorXd const &ub, int npoints, int first_element): m_lb_pars(lb),m_ub_pars(ub)
{
  ...
  for(int i=0;i<npoints;i++){
    VectorXd theta=lb+(ub-lb)*H.col(i);
...
void Density::Run_FullMCMCWithScale(int nsteps,int nsamples, MatrixXd const & COV_init,default_random_engine & generator){
//MCMC à hpars variables
  m_samples.clear(); m_hparsofsamples.clear(); m_allmcmcsamples.clear();
  int dim_mcmc=m_dim_pars+m_dim_hpars;
  auto compute_score=[this](VectorXd const & X)-> double {
  ...
  MatrixXd COV=COV_init;
  double accrate=Run_Burn_phase_FullMCMC(nsteps*0.1,COV,Xinit,generator);
  COV=scale_covmatrix(COV,Xinit,compute_score,0,generator);
  MatrixXd sqrtCOV=COV.llt().matrixL();
  double finit=compute_score(Xinit);
  VectorXd Xcurrent=Xinit;
  double fcurrent=finit;
...
MatrixXd Density::scale_covmatrix(MatrixXd const & COV, VectorXd const & Xcurrent, function<double(VectorXd)> const & compute_score, double true_accrate, default_random_engine & generator) const{
  ...
  ofstream ofile("results/scaling.gnu");
