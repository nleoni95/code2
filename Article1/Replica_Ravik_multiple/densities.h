...
public:
    ...
    std::vector<Eigen::VectorXd> Draw_likelihood_samples(Eigen::VectorXd const & theta,Eigen::VectorXd const & hpars, int nsamples,std::default_random_engine & generator) const;

    double Compute_reference_prior(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars, int nsamples_ll, int niter,std::default_random_engine & generator) const;
