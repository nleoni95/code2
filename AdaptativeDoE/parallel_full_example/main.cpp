...
double EI_prob(GP &gp, VectorXd thetaprime, VectorXd hpars_typical, MCMC_opti &mcmc, vector<DATA> &yobs, default_random_engine &generator, double max_courant_logpost){
...
double EI_prob_noedm(GP &gp, VectorXd thetaprime, VectorXd hpars_typical, MCMC_opti &mcmc, vector<DATA> &yobs, default_random_engine &generator, double max_courant_logpost){
  //calcule l'EI de la log-probabilité d'amélioration, au point thetaprime.
  //on créée les données (obs-gp(theta)) qu'on met dans un vector <DATA> de taille nobs
  int dimx=yobs[0].GetX().size(); int dimtheta=thetaprime.size(); int dimgp=dimx+dimtheta;
  vector<DATA> data2;
...
int main( int argc, char **argv){
  ...
  {
    int tid, nthreads;
    // Obtenir le numero du thread en question
    tid = omp_get_thread_num();
    printf("Bonjour de la part du thread %d\n", tid);
    // Obtenir le nombre total de threads
	  nthreads = omp_get_num_threads();
	  cout << "Il y a" << nthreads << "thread(s)" << endl;
  }
  exit(0);
  // Inputs:
  ...
  for (int i=0;i<nobs_max;i++){
    ...
    for (const auto &theta:lhs1){
      double e=EI_prob(gp1,theta,hpars_typical,mcmc,yobs,generator,ll_max1);
      //cout << e << endl;
      if(e>crit_max){
    ...
    double l=mcmc.loglikelihood_theta(&yobs,hpars_opti,theta_candidate);
    cout << "l :" << l << endl;
    if(l>ll_max1){ll_max1=l;}
    ...
    double d=test_gp(gp1,lb_t,ub_t,lb_x,ub_x);
    fprintf(out,"%e\n",d);
  }
  fclose(out);
  ...
  out=fopen("results/L2noedm.gnu","w");
    ...
    for (const auto &theta:lhs2){
      double e=EI_prob_noedm(gp2,theta,hpars_typical,mcmc,yobs,generator,ll_max2);
      if(e>crit_max){
        theta_candidate=theta;
        crit_max=e;
      }
  ...
  for (int i=0;i<lhs.size();i++){
    double e=EI_prob(gp1,lhs1[i],hpars_typical,mcmc,yobs,generator,ll_max1);
    double f=EI_prob_noedm(gp2,lhs2[i],hpars_typical,mcmc,yobs,generator,ll_max2);
    fprintf(out,"%e %e %e\n",lhs1[i](0),e,f);
  }
  fclose(out);
