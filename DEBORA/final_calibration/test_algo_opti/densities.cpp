...
double Density::loglikelihood_theta_fast(Eigen::VectorXd const &theta,Eigen::VectorXd const &hpars, Eigen::LDLT<Eigen::MatrixXd> const &ldlt)const{
    ...
    VectorXd obs=Vals-m_model(m_data_exp[0].GetX(),theta);//-m_priormean(m_data_exp[0].GetX(),hpars); //problème ici. On voudrait mettre la priormean, sauf que ça dépend d'hpars qui ne sont pas connus dans cette fonction. On pourrait le corriger en créant les data dans le fonction d'avant, mais cela ne permettrait pas le découplage pour optfunc_opt. On reste sans priormean.
...
double Density::KDE(VectorXd const & theta, VectorXd const &window) const{
  //estimation de la densité KDE en un point theta.
  double f=0;
  double windowsize=1;
  for(int i=0;i<window.size();i++){
    windowsize*=window(i);
  }
  for(const VectorXd &s:m_samples){
    VectorXd x=(theta-s).cwiseQuotient(window);
    f+=exp(-0.5*x.squaredNorm());
  }
  return f/(m_samples.size()*windowsize*pow(2*M_PI,window.size()/2.0));
}

VectorXd Density::GetKDEWindow()const {
  //estimation automatique de la window: calcul des stds dans chaque composante des m_samples; fenêtre de Wikipédia.
  VectorXd means=VectorXd::Zero(m_dim_pars);
  VectorXd stds=VectorXd::Zero(m_dim_pars);
  for(auto &v:m_samples){
    means+=v;
    stds.array()+=v.array().square();
  }
  int n=m_samples.size();
  stds.array()=stds.array()/n - (means/n).array().square();
  stds.array()=stds.array().sqrt();
  return 1.06*stds*pow(n,-1./9);
}
...
void Density::WritePredictionsF(VectorXd const & X, string const & filename) const{
  ...
  //On affiche la prédiction en theta mean posterior.
  cout << "computing predictions with " << m_samples.size() << " samples..." << endl;
  //calcul du thetameanmap :
  VectorXd thetameanpost=VectorXd::Zero(m_dim_pars);
  for(int i=0;i<m_samples.size();i++){
    thetameanpost+=m_samples[i];
  }
  thetameanpost/=m_samples.size();
  cout << "theta mean post : " << thetameanpost.transpose() << endl;
  ...
  VectorXd predf_meanpost=m_model(X,thetameanpost);
  ...
  ofstream ofile(filename);
  for(int i=0;i<meanf.rows();i++){
    ofile << X(i) <<" "<< meanf(i) << " " << quant25(i) << " " << quant75(i) << " " << quant2p5(i) << " " << quant97p5(i) << " " << predf_meanpost(i) << endl;
  }
  ofile.close();
}
...
double DensityOpt::EstimatePredError(VectorXd const & theta) const {
  ...
  return EvaluateVarHparOpt(theta).array().sum();
  }
...
double DensityOpt::compute_evidence(int npoints, default_random_engine & generator)const{
  uniform_real_distribution<double> distU(0,1);
  ...
  double integral=0;
  for(int i=0;i<npoints;i++){
    VectorXd hpars=EvaluateHparOpt(thetas[i]);
    integral+=exp(loglikelihood_theta(thetas[i],hpars)+m_logpriorhpars(hpars));
  }
...
Eigen::VectorXd DensityOpt::Test_hGPs_on_sample(std::vector<Eigen::VectorXd> const & theta_ref,std::vector<Eigen::VectorXd> const & hpars_ref, string const & filename) const{
  ...
  //valeur rms
  errmoy/=true_logliks.size(); //estimateur de la DKL
...
void DensityOpt::BuildHGPs_noPCA(double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs){
  ...
  vector<GP> vgp(nmodes);
  //distinction entre premier appel à la fonction ou non
...
void DensityOpt::update_hGPs_noPCA(vector<VectorXd> const &new_thetas,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs, double max_time){
  //on fait une optimisation fine sur les new thetas, puis on les rajoute aux hGPs. On optimise les hyperparamètres avec le tout.
  cout << "updating hGPs with " << new_thetas.size() << "new points..." << endl;
  vector<VectorXd> new_hpars(new_thetas.size());
  VectorXd hpars_guess=0.5*(m_lb_hpars+m_ub_hpars);
...
void DensityOpt::update_hGPs_noPCA(vector<VectorXd> const &new_thetas,vector<VectorXd> const &new_hpars,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs){
  //on fait une optimisation fine sur les new thetas, puis on les rajoute aux hGPs. On optimise les hyperparamètres avec le tout.
  cout << "updating hGPs with " << new_thetas.size() << "new points with given hpars" << endl;
  VectorXd hpars_guess=0.5*(m_lb_hpars+m_ub_hpars);
  for(int i=0;i<new_thetas.size();i++){
    AUGDATA dat; dat.SetX(new_thetas[i]),dat.SetValue(new_hpars[i]);
    m_hpars_opti.push_back(dat);
...
void DensityOpt::update_hGPs(vector<VectorXd> const &new_thetas,vector<VectorXd> const &new_hpars,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes){
  //on rajoute les new_thetas aux hGPs. Inclus : optimisation des hpars hgps.
