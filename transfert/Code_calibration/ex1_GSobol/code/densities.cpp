...
void Density::SetFixedInputerr(double logstdinputerr, VectorXd true_derivatives_at_obs)
{
  m_presence_inputerr = 1;
  m_inputerr = logstdinputerr;
  VectorXd derivativessquared = true_derivatives_at_obs.array().square();
  m_derivatives_obs = derivativessquared.asDiagonal();
}
void Density::SetLearnedInputerr(int index, VectorXd true_derivatives_at_obs)
{
...
void Density::WritePredictions(vector<VectorXd> const &X, string const &filename) const
{
   ...
   for (int i = 0; i < Predictions.rows(); i++)
  {
    ...
    ofile << Predictions(i, 0) << " " << sqrt(Predictions(i, 1) + Predictions(i, 2)) << " " << meanf(i) << " " << quant2p5(i) << " " << quant97p5(i) << endl;
  }
  ofile.close();
}
