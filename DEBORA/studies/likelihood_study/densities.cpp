...
double Density::loglikelihood_fast_terme_fit(VectorXd const &obs,LDLT<MatrixXd> const &ldlt) const{
    //calcul de la LL à la dernière étape.
    VectorXd Alpha=ldlt.solve(obs);
    int nd=obs.size();
    return -0.5*obs.dot(Alpha);
}

double Density::loglikelihood_fast_terme_complexity(VectorXd const &obs,LDLT<MatrixXd> const &ldlt) const{
    //calcul de la LL à la dernière étape.
    VectorXd Alpha=ldlt.solve(obs);
    int nd=obs.size();
    return -0.5*(ldlt.vectorD().array().log()).sum();
}

double Density::loglikelihood_theta_fast_decomposition(std::vector<AUGDATA> const &exp_data, Eigen::VectorXd const &theta, Eigen::LDLT<Eigen::MatrixXd> const &ldlt)const{
    ...
    double l1=loglikelihood_fast_terme_complexity(obs,ldlt);
    double l2=loglikelihood_fast_terme_fit(obs,ldlt);
    cout << "terme complexity : " << l1 << endl;
    cout << "terme fit : " << l2 << endl;
    cout << "-fit L2 : " << -0.5*obs.squaredNorm() << endl;
    return l1+l2;
}
...
double Density::loglikelihood_theta_decomposition(std::vector<AUGDATA> const &exp_data, Eigen::VectorXd const &theta, Eigen::VectorXd const &hpar)const{
    //évaluation de la LDLT de Gamma. a priori écrite pour un AUGDATA de taille 1 également. Si la taille change, on pourrait faire plusieurs gamma (plusieurs hpars.)
    MatrixXd G=Gamma(m_Xprofile_converted,hpar);
    LDLT<MatrixXd> ldlt(G);
    return loglikelihood_theta_fast_decomposition(exp_data,theta,ldlt);
}

void Density::loglikelihood_study(VectorXd const & theta, VectorXd const & hpars) const{
  //on calcule précisément les différents termes de la vraisemblance d'un calcul.
  cout << "theta : " << theta.transpose() << endl; 
  cout << "hpars : " << hpars.transpose() << endl;
  double l=loglikelihood_theta_decomposition(m_data_exp,theta,hpars);
  int nd=m_data_exp[0].Value().size();
  double cst=-0.5*nd*log(2*3.1415);
  cout << "sum : " << l+cst << endl;
  double lp=m_logpriorhpars(hpars);
  cout << "logprior : " << lp;
  cout << "sum : " << l+lp << endl;
  VectorXd fpred=m_model(VectorXd::Random(1),theta);
  cout << "f : coeur, milieu, paroi : " << fpred(0) << " " << fpred(20) << " " << fpred(35) << endl;
  cout << endl;
}

void Density::loglikelihood_grid(VectorXd const & theta, VectorXd const & lb_hparsgrid, VectorXd const & ub_hparsgrid, double sobs, int ngrid, string filename) const{
  //construction d'un grid de hpars. log-scale en sigma_edm.
  vector<VectorXd> grid_hpars;
  for(int i=0;i<ngrid;i++){
    for(int j=0;j<ngrid;j++){
      VectorXd h(3);
      h(1)=sobs;
      h(0)=exp(log(lb_hparsgrid(0))+(log(ub_hparsgrid(0))-log(lb_hparsgrid(0)))*i/(1.0*ngrid));
      h(2)=exp(log(lb_hparsgrid(1))+(log(ub_hparsgrid(1))-log(lb_hparsgrid(1)))*j/(1.0*ngrid));
      grid_hpars.push_back(h);     
    }
  }
  cout << "évaluation de la ll..." << endl;
  //évaluation des différents termes.
  vector<double> l_fit;
  vector<double> l_compl;
  for(int i=0;i<grid_hpars.size();i++)
    {
      VectorXd hpar=grid_hpars[i];
      MatrixXd G=Gamma(m_Xprofile_converted,hpar);
      LDLT<MatrixXd> ldlt(G);
      VectorXd X=m_data_exp[0].GetX();
      VectorXd Vals=m_data_exp[0].Value();
      VectorXd obs(Vals.size());
      obs=Vals-m_model(X,theta); 
      double l1=loglikelihood_fast_terme_complexity(obs,ldlt);
      double l2=loglikelihood_fast_terme_fit(obs,ldlt);
      l_fit.push_back(l2);
      l_compl.push_back(l1);
    }
  //écriture dans un fichier
  ofstream ofile(filename);
  for(int i=0;i<grid_hpars.size();i++){
    ofile << grid_hpars[i](0) << " " << grid_hpars[i](2) << " " << l_fit[i] << " " << l_compl[i] << " " << l_fit[i]+l_compl[i] << endl;
  }
  ofile.close();
}
...
void DensityOpt::Test_hGPs() {
  ...
  if(m_newgrid.size()==0){
    ...
    for(int i=0;i<nthetas;i++){
      Hopt.col(i)=HparsOpt_quick(newgrid[i],hpars_guess);
    }
    m_newgrid=newgrid;
    m_Hopt_newgrid=Hopt;
  }
