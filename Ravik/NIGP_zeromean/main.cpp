...
int main(int argc, char **argv){
  ...
  int nombre_samples_collected=500; //1 sample tous les 200. Sinon trop long...
  int nautocor=2000;
  ...
  VectorXd ub_hpars(2); ub_hpars(0)=2e6;ub_hpars(1)=25; //bornes sur l'optimisation des hpars. edm exp lcor
  ...
  vector<int> cases={3,4,5,6,8,14,15,16,18,20,21,22,23,24}; // le cas 18 est puni.
...
exit(0);
  ...
  //COV_init=pow(0.05,2)*MatrixXd::Identity(3,3);
...
for(int i=0;i<vsize;i++){
    ...
    auto compute_score_fb=[vsize,lb_hpars,vDens,i](vector<VectorXd> p, VectorXd const &X){
      ...
      double lp=vDens[i].EvaluateLogPHpars(hp);
      return d+lp;
    }; 
    cout << "début mcmc densité bayes expérience" << cases[i] << endl;
      
    auto res=Run_MCMC(3*nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc_fb,COV_init_fb,compute_score_fb,get_hpars_fb,in_bounds_fb,generator);
