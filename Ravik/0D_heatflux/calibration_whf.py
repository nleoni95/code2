...
# dans ce framework : on adapte les fonctions pour pouvoir être utilisées dans un cadre de calibration.
# les coefficients à calibrer vont prendre la variable 
...
def print_predictions(indice,param):
    ...
    for i in np.arange(200):
        Y[i]=chaleur(selected_case['p'].iloc[0],X[i],selected_case['DTsub'].iloc[0],selected_case['v'].iloc[0],selected_case['diam'].iloc[0],param)
        YRavik[i]=chaleur(selected_case['p'].iloc[0],X[i],selected_case['DTsub'].iloc[0],selected_case['v'].iloc[0],selected_case['diam'].iloc[0],ParamsRavik)
    ax=plt.subplot(111)
    plt.plot(X,Y,label='Prediction fittée',color='tab:blue')
    plt.plot(X,YRavik,label='Prediction nominale',color='black')
    plt.scatter(data_points[:,0],data_points[:,1],color='tab:red',label='Exp data')
    ax.set_xlabel('Wall superheat [K]')
    ax.set_ylabel('Wall heat flux [W/m2]')
    ax.set_ylim([0,data_points[-1,1]])
    plt.title('case : '+str(indice))
    plt.legend()
    plt.grid()
    plt.show()
...
res=opt.minimize(performance01,X0,method='Powell',options={"maxiter":10000},bounds=bounds)
optresult=res.x
paramsopti=copy.deepcopy(ParamsRavik)
paramsopti[0]=optresult[0]
paramsopti[1]=optresult[1]
paramsopti[3]=optresult[2]
print('valeurs optimisées : '+str(optresult[0])+', '+str(optresult[1])+', '+str(optresult[2]))
print('valeurs initiales : '+str(ParamsRavik[0])+' ,'+str(ParamsRavik[1])+' ,'+str(ParamsRavik[3]))
print('cas de calibration : '+str(caselol))

print(pred_capability01(caselol,paramsopti))
print_predictions(caselol,paramsopti)
