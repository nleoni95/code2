...
double INTERPOL::fint(double x, int i, int l){
	double sum1=1;
	double sum2=1;
	for(int j=0; j<Npt(l); j++){
		if(j!=i){
			sum1 *= (x-Xp(Xind(j,l)));
			sum2 *= (Xp(Xind(i,l)) - Xp(Xind(j,l)));
		}
	}
	return sum1/sum2;
};

void INTERPOL::prep_fejer(){
	Npt(0) = 1;					//number of points
	Npn(0) = 1;					//number of new points
...
//Prepare weights:
	...
	Wghto = Wght;
//	for(int il=nl-1; il>0; il--) for(int i=0; i<Npt(il-1); i++) Wght(2*i+1,il) -= Wght(i,il-1);
