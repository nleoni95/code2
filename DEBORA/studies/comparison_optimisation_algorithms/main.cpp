...
//Comparaison des performances optimisation par gradients / sans gradients.
...
int main(int argc, char **argv){
  ...
  //comparaison des optimisations algorithms.
  {
       
    DensityOpt DensOpt_diam(MainDensity_diam);
    VectorXd theta_test(5);
    theta_test << .5,.5,.5,.5,.5;
    //0.000265342 7.58255e-07  0.00162244 0.000257531
    //maximum trouvé. valeur : 475.44
    ofstream ofileg("results/optigrad.gnu");
    ofstream ofileng("results/optinograd.gnu");

    auto optig=[&DensOpt_diam,&theta_test,&hpars_z_guess_diam,&ofileg](double tol){
      int nevals=0;
      cout << "begin opti with gradients. tol = " << tol << endl;
      auto begin=chrono::steady_clock::now();
      VectorXd h=DensOpt_diam.HparsOpt(theta_test,hpars_z_guess_diam,tol,nevals) ;
      auto end=chrono::steady_clock::now();
      cout << "arg : " << h.transpose() << endl;
      double max=DensOpt_diam.loglikelihood_theta(theta_test,h)+DensOpt_diam.EvaluateLogPHpars(h);
      cout << "max value : " << max << endl;
      double t=chrono::duration_cast<chrono::milliseconds>(end-begin).count();
      cout << "time : " << t  << " ms." << endl;
      ofileg << tol << " " << h(0) << " " << h(1) << " " << h(2) << " " << h(3) << " " << max << " " << nevals << " " << t << endl;
    };

    auto opting=[&DensOpt_diam,&theta_test,&hpars_z_guess_diam,&ofileng](double tol){
      int nevals=0;
      cout << "begin opti no gradients. tol = " << tol << endl;
      auto begin=chrono::steady_clock::now();
      VectorXd h=DensOpt_diam.HparsOpt_nograd(theta_test,hpars_z_guess_diam,tol,nevals) ;
      auto end=chrono::steady_clock::now();
      cout << "arg : " << h.transpose() << endl;
      double max=DensOpt_diam.loglikelihood_theta(theta_test,h)+DensOpt_diam.EvaluateLogPHpars(h);
      cout << "max value : " << max << endl;
      double t=chrono::duration_cast<chrono::milliseconds>(end-begin).count();
      cout << "time : " << t  << " ms." << endl;
      ofileng << tol << " " <<h(0) << " " << h(1) << " " << h(2) << " " << h(3) << " " << max << " " << nevals << " " << t << endl;
    };
