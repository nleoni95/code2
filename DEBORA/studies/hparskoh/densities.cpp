...
void Density::Run_MCMC_fixed_hpars(int nsteps,int nsamples, MatrixXd const & COV_init,VectorXd const & hpars,default_random_engine & generator){
  ...
  while(accrate<0.20){
    ...
    accrate=Run_Burn_phase_MCMC(nsteps*0.1,COV,hpars,Xinit,generator);
    FindVPs(COV);
    COV=COVPred;
  }
...
//MCMC à hpars variables
  ...
  COV=scale_covmatrix(COV_init,Xinit,compute_score,0,generator);
  ...
  while(accrate<0.20){
    ...
    COVPred=COV;
    m_allmcmcsamples.clear();
    accrate=Run_Burn_phase_FullMCMC(nsteps*0.1,COV,Xinit,generator);
    FindVPs(COV);
    COV=COVPred;
  }
  */
...
MatrixXd Density::scale_covmatrix(MatrixXd const & COV, VectorXd const & Xcurrent, function<double(VectorXd)> const & compute_score, double true_accrate, default_random_engine & generator) const{
  ...
  cout << endl << "début du scaling de la covmatrix." << endl;
  cout << "covmatrix : " << endl << COV <<endl;
...
void DensityOpt::Test_hGPs(default_random_engine & generator) {
  ...
  MatrixXd Hopt(3,nthetas);
  for(int i=0;i<nthetas;i++){
    Hopt.col(i)=HparsOpt(newgrid[i],hpars_guess);
  }
  MatrixXd Hoptpred(3,nthetas);
  for(int i=0;i<nthetas;i++){
