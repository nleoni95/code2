...
class Density
{
  public:
    ...
    //fonctions d'optimisation d'hpars

    Eigen::VectorXd Opti_hpars(Eigen::VectorXd &theta,Eigen::VectorXd &hpars_guess) const; //renvoie les hyperparamètres optimaux à une fonction de theta. appelle le modèle à la valeur theta.
    static double optfunc_d(std::vector<double> const &x, std::vector<double> &grad, void *data);
...
class DensityKOH : public Density
{
  public:
  DensityKOH(DoE g);
  DensityKOH(Density const &d, int c);
...
{
  public:
  DensitySimple(Density const &d, double c);
  void Build();
  protected:
...
class MCMC : public Density{
  ...
  public:
  MCMC(Density const &d,int nchain);
