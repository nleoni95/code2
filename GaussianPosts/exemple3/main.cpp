...
int main(int argc, char **argv){
  ...
  auto run_analysis=[](string foldname,int nobs,int seed){
    ...
    VectorXd ub_t(dim_theta); ub_t(0)=0;

    //hpars. hpars(1) est forcément la sigma exp.
    VectorXd lb_hpars(dim_hpars); lb_hpars(0)=1e-4;lb_hpars(1)=1e-4;lb_hpars(2)=1e-2;
    VectorXd ub_hpars(dim_hpars); ub_hpars(0)=5;ub_hpars(1)=5;ub_hpars(2)=5;
    ...
    auto true_fct=[&generator](VectorXd const & xobs){
      VectorXd res(xobs.size());
      double noise=sqrt(0.1);
      for(int i=0;i<res.size();i++){
        double x=xobs(i)*2*M_PI;
        res(i)=sin(x)*exp(x/10)+noise*distN(generator);
      }
      return res;
    };
    ...
    COV_init(2,2)=pow(5e-2,2);
    COV_init(3,3)=pow(5e-2,2);
