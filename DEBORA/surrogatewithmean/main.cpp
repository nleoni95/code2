...
int main(int argc, char **argv){
  ...
  //diam
  lb_hpars << 1e-8,1e-8,2e-3,1e-5; 
  ub_hpars << 5e-4,5e-4,2e-1,5e-4;
  ...
  int nhpars_gp=7;
  MatrixXd Bounds_hpars_gp(2,nhpars_gp);
  Bounds_hpars_gp(0,0)=1E-1; Bounds_hpars_gp(1,0)=1e1; //variance
  Bounds_hpars_gp(0,2)=1E-4; Bounds_hpars_gp(1,2)=1e1; //sigma obs
  list<int> l={1,3,4,5,6};
  for (int i:l){
    Bounds_hpars_gp(0,i)=1E-2;Bounds_hpars_gp(1,i)=2; //lcors.
  }
  ...
  hpars_gp_guess << 1,0.5,0.001,0.5,0.5,0.5,0.5;

  // pour la MCMC
  MatrixXd COV_init=pow(0.1,2)*MatrixXd::Identity(5,5);
  COV_init*=0.25;
  MatrixXd COV_init_bayes=MatrixXd::Identity(9,9);
  COV_init_bayes.topLeftCorner(5,5)=COV_init;
  COV_init_bayes(5,5)*=pow(1e-6,2); //sedm
  COV_init_bayes(6,6)*=pow(1e-6,2); //sobs
  COV_init_bayes(7,7)*=pow(5e-5,2); //lcor
  COV_init_bayes(8,8)*=pow(5e-5,2);
   //offset
  COV_init_bayes*=0.5;
  ...
  DoE doe_init(lb_t,ub_t,5);//5000,generator);//3125 pts 
  ...
  MainDensity.WriteMCMCSamples("results/diag/allsampkohmat5.gnu");
  MainDensity.WriteSamples("results/diag/sampkohmat5.gnu");
  auto mcmc_koh=chrono::steady_clock::now();

  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamkohmat5F.gnu");
...
/*
  ...
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamnoedmmat5F.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/diamnoedmmat5.gnu");
  MainDensity.WriteSamples("results/save/samplesnoedmdiammat5.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocornoedmmat5.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampnoedmmat5.gnu");
  ...
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamfullbayesmat5F.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/diamfullbayesmat5.gnu");
  MainDensity.WriteSamples("results/save/samplesfullbayesdiammat5.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocorfullbayesmat5.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampfullbayesmat5.gnu");
  ...
  */
  ...
  DensOpt.Initialise_GPs(Kernel_GP_Matern,Bounds_hpars_gp,hpars_gp_guess);
  //DensOpt.opti_allgps(hpars_gp_guess);
  DensOpt.Test_hGPs(generator);
  DensOpt.WritehGPs("results/save/hGPsdiammat5.gnu");
  ...
  DensOpt.Autocor_diagnosis(nautocor,"results/diag/autocoroptmat5.gnu");
  DensOpt.WriteMCMCSamples("results/diag/allsampoptmat5.gnu");

  //DensOpt.ReadSamples("results/save/sampoptdiammat.gnu");

  DensOpt.WritePredictionsF(VectorXd::Random(1),"results/preds/diamoptmat5F.gnu");
  DensOpt.WritePredictions(VectorXd::Random(1),"results/preds/diamoptmat5.gnu");

  auto pred_opt=chrono::steady_clock::now();

  DensOpt.WriteSamples("results/save/sampoptdiammat5.gnu");
