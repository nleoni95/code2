...
void DensitiesOpt::read_optimal_hpars(vector<vector<VectorXd>> & hparsv, vector<VectorXd> & grid_theta,string filename) const {
  ...
  if(ifile){
    ...
    while(getline(ifile,line)){
      istringstream iss(line);
      vector<string> words((istream_iterator<string>(iss)),istream_iterator<string>());
      int number_cases=words.size()/3-1;
      if(!number_cases==ncases){cerr << "erreur dans ncases" << endl;}
      VectorXd theta(3);
      ...
      for(int i=0;i<number_cases;i++){
        VectorXd hpars(3); //a modifier...
        for(int j=0;j<3;j++){
          hpars(j)=stod(words[3+j+i*3]);
        }
