...
void Run_Burn_Phase_MCMC(int nburn, MatrixXd &COV_init, VectorXd &Xcurrento, function<double(vector<VectorXd>, VectorXd const &)> const &compute_score, function<vector<VectorXd>(VectorXd const &)> const &get_hpars, function<bool(VectorXd)> const &in_bounds, default_random_engine &generator)
{
  ...
  cout << "finit : " << fcurrent << endl;
  cout << "thetainit : " << Xcurrent.transpose() << endl;
  cout << "hparsinit : " << hparscurrent[0].transpose() << endl;
...
vector<VectorXd> Run_MCMC(int nsteps, VectorXd &Xinit, MatrixXd &COV_init, function<double(vector<VectorXd>, VectorXd const &)> const &compute_score, function<vector<VectorXd>(VectorXd const &)> const &get_hpars, function<bool(VectorXd)> const &in_bounds, default_random_engine &generator)
{
  ...
  cout << "thetacurrent : " << Xcurrent.transpose() << endl;
  cout << "hparscurrent[0] : " << hparscurrent[0].transpose() << endl;
  int naccept = 0;
  auto begin = chrono::steady_clock::now();
...
VectorXd DensityOpt::HparsOpt(VectorXd const &theta, VectorXd hpars_guess, double max_time) const
{
  ...
  //évaluation du critère. Si jamais le critère avec l à la borne min. est un tout petit peu moins bon que le critère optimisé, on prend la borne min. ça facilite le choix.
  vector<double> xbest=VXDtoV(hpars_guess);
  vector<double> grad;
  VectorXd hparstest=hpars_guess;
  hparstest(1)=m_lb_hpars(1);
  vector<double> xtest=VXDtoV(hparstest);
  double critopt=optfuncOpt_nograd(xbest,grad,&p);
  double crittest=optfuncOpt_nograd(xtest,grad,&p);
  if(critopt-crittest<1e-2){return hparstest;}
  return hpars_guess;
}
