...
double D1Kernel_Z_SE(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  ...
  double d=abs(x(0)-y(0));
  //return pow(hpar(0),2)*exp(-d/hpar(2));
  return 2*hpar(0)*exp(-0.5*pow(d/hpar(2),2)); //5/2
}
...
double D3Kernel_Z_SE(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  ...
  double X=d/hpar(2);
  //return pow(hpar(0),2)*exp(-d/hpar(2));
  return -0.25*X*pow(hpar(0),2)*exp(-0.5*pow(X,2));
}
...
int main(int argc, char **argv){
  ...
  //diam
  lb_hpars << 2e-5,1e-8,5e-4; 
  ub_hpars << 5e-3,3e-5,1e-1;
  ...
  COV_init(2,2)=pow(0.05,2);
  MatrixXd COV_init_bayes=MatrixXd::Identity(8,8);
  COV_init_bayes.topLeftCorner(5,5)=COV_init;
  COV_init_bayes(5,5)*=pow(1e-4,2); //sedm avant 1e-7
  COV_init_bayes(6,6)*=pow(1e-7,2); //sobs avat 1e-8
  COV_init_bayes(7,7)*=pow(1e-3,2); //lcor avant 1e-3
...
/*
  ...
  */
  ...
  double sobs=6e-7;
  VectorXd lb_hparsgrid(2);
  VectorXd ub_hparsgrid(2);
  lb_hparsgrid << 5e-5,5e-4;
  ub_hparsgrid << 1e-2,5e-2;
  ...
  /*
  VectorXd hpars1=DensOpt.HparsOpt(theta1,hpars_koh);
  VectorXd hpars2=DensOpt.HparsOpt(theta2,hpars_koh);
  VectorXd hpars3=DensOpt.HparsOpt(theta3,hpars_koh);
  VectorXd hpars4=DensOpt.HparsOpt(theta4,hpars_koh);
  

  DensOpt.loglikelihood_study(theta1,hpars1);
  DensOpt.loglikelihood_study(theta2,hpars2);
  DensOpt.loglikelihood_study(theta3,hpars3);
  DensOpt.loglikelihood_study(theta4,hpars4);
  */
 DensOpt.loglikelihood_grid(theta1,lb_hparsgrid,ub_hparsgrid,sobs,100,"results/gridt1.gnu");

 DensOpt.loglikelihood_grid(theta2,lb_hparsgrid,ub_hparsgrid,sobs,100,"results/gridt2.gnu");
  exit(0);
