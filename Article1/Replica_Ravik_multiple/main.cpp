...
int main(int argc, char **argv)
{
  ...
  { //paramètres de l'algorithme iterative sampling.
    ...
    int niter = 2;
    double time_opti_fine = 5e-1; //sans gradients.
...
exit(0);
      ...
      // calibration BAYES_backupsingle
  {
    ...
    cout << "run mcmc from starting point XinitMCMC. COV init from file." << endl;
    //COV_fromFMP /= 25;
    vector<VectorXd> allsteps = Run_MCMC_hundred(10 * nsteps_mcmc, starting_points[1], COV_fromFMP, compute_score_fb, get_hpars_fb, in_bounds_fb, generator);
    ...
    //diagnostic
    Selfcor_diagnosis(allsteps, 5000, 0.5, "results/bayes/autocor.gnu");
