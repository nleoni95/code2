...
int main(int argc, char **argv){
  ...
  ///phase opti
      
    {
      ...
      //étude sur les dérivées du kernel. je deviene fou.
      /*
      {
        auto f=[&hpars_z_guess_diam](double d,double l){
          VectorXd x(1);
          VectorXd y(1);
          x(0)=0;y(0)=d;
          VectorXd h=hpars_z_guess_diam;
          h(2)=l;
          return Kernel_Z_Matern52(x,y,h);
        };
        auto fp=[&hpars_z_guess_diam](double d,double l){
          VectorXd x(1);
          VectorXd y(1);
          x(0)=0;y(0)=d;
          VectorXd h=hpars_z_guess_diam;
          h(2)=l;
          return D3Kernel_Z_Matern52(x,y,h);
        };
        const double d=6e-7;
        const double lmin=5e-7;
        const double lmax=7e-7;
        const int npts=100;
        ofstream ofile ("results/f.gnu");
        for(int i=0;i<npts;i++){
          double l=lmin+(lmax-lmin)*(1.0*i)/npts;
          double dl=1e-11;
          ofile << l << " " << f(d,l) << " " << fp(d,l) << " " << (f(d,l+dl)-f(d,l-dl))/(2*dl) << endl;
        }
        ofile.close();
        exit(0);
      }
