  ...
  transform(prob1.begin(), prob1.end(), prob1.begin(), [logvstyp](const double &d) -> double
            {
              ...
              if (isinf(f))
              {
                cerr << "erreur myoptfunc_koh : infini prob1" << endl;
              }
              return f;
            });
  transform(prob2.begin(), prob2.end(), prob2.begin(), [logvstyp](const double &d) -> double
            {
              ...
              if (isinf(f))
              {
                cerr << "erreur myoptfunc_koh : infini prob2" << endl;
              }
              return f;
            });
...
int main(int argc, char **argv)
{
...

  {
    ...
    vector<AUGDATA> data_exp_alpha(1);
    {
      AUGDATA dat;
      dat.SetX(Xgrid_num);
      dat.SetValue(Yexpe_interpol_diam);
      data_exp_diam[0] = dat;
    ...
    MainDensity_alpha.SetKernelDerivatives(D1Kernel_Z_Matern52, D2Kernel_Z_Matern52, D3Kernel_Z_Matern52);
    ...
    MainDensity_diam.SetKernelDerivatives(D1Kernel_Z_Matern52, D2Kernel_Z_Matern52, D3Kernel_Z_Matern52);
...

    {
      ...
      auto eval = [&DensOpt_alpha, &DensOpt_diam,&hpars_z_guess_alpha,&hpars_z_guess_diam](VectorXd const &X,double time)
      {
        cout << "time : " << time << endl;
        VectorXd hparsopt_alpha = DensOpt_alpha.HparsOpt(X,hpars_z_guess_alpha,time);
        VectorXd hparsopt_diam = DensOpt_diam.HparsOpt(X,hpars_z_guess_diam,time);
        ...
        cout << "score : " <<  ll1 + ll2 + lp << endl << endl;
      };
      vector<double> v={1e-4,1e-3,1e-2,1e-1,1};
      for (auto time:v){
        eval(VectorXd::Ones(5),time);
      }
      ...
      VectorXd v1(3); v1 << 0.191809,0.00214387,0.002891;
      VectorXd v2(3); v2 << 0.000302079,7.61368e-7,0.00301606;
      cout << compute_score_opti(make_pair(v1,v2),VectorXd::Ones(5)) << endl;
      exit(0);
    }
