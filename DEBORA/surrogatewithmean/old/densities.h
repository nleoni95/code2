...
class DoE
{
 public:
  ...
  void Fill(std::function<Eigen::VectorXd(Eigen::VectorXd const &)> my_model,std::vector<DATA> *obs); //construit le DoE avec un modèle et observations données.
  void Fill_Predictions(std::vector<double> &vectorx); //fait l'évaluation du modèle sur les points de la prédictions. Il faut être malin ici.
  
  void SetModel(std::function<Eigen::VectorXd(Eigen::VectorXd const &)> my_model){m_my_model=my_model;}
  ...
  std::vector<double> GetVectorX() {return m_vectorx;};
 ...
 private:
  ...
  std::vector<double> m_vectorx; //vecteur d'évaluation du modèle
  std::vector<DATA> m_obs; //observations ici
  std::function<Eigen::VectorXd(Eigen::VectorXd const &)> m_my_model;
...
class Density
{
  public:
    ...
    void SetKernel(double (*Kernel)(double, double, Eigen::VectorXd const &)){m_Kernel=Kernel;};
    
    void SetModel(std::function<Eigen::VectorXd(Eigen::VectorXd const &)> my_model){m_my_model=my_model;}
    void SetPriorMean(double (*PriorMean)(double,Eigen::VectorXd const &)){m_priormean=PriorMean;}
    ...
    Eigen::VectorXd MyModel(Eigen::VectorXd const & pars)const{return m_my_model(pars);};
    ...
    double (*m_priormean)(double,Eigen::VectorXd const &); //x et hpars
  ...
  protected:
    ...
    double (*m_Kernel)(double, double, Eigen::VectorXd const &);
    double (*m_logpriorhpars)(Eigen::VectorXd const &);
    double (*m_logpriorpars)(Eigen::VectorXd const &);
    //Eigen::VectorXd (*m_my_model)(Eigen::VectorXd const &);
    std::function<Eigen::VectorXd(Eigen::VectorXd const &)> m_my_model;
...
class DensitySimple : public DensityKOH
{
  ...
  void Build();
  protected:
  static double KernelNull(double x, double xp, Eigen::VectorXd const &hpar){return 0;}
