...
Density::Density(DoE g) : m_Grid(g),m_npts(g.GetGrid().size()),m_dim_pars(g.GetDimension()),m_model_evals(g.GetModelEvals()),m_obs(g.GetObs())
{
...
MatrixXd Density::VarParamUncertainty(vector<VectorXd> &vectorx) const {
  ...
  for (int i=0;i<xsize;i++)
  {
    ...
    for (int j=0;j<m_npts;j++){
      if(m_filtre[j]){
        X(c)=m_my_model(vectorx[i],m_Grid.GetGrid()[j]); //mauvais
        ++c;
      }
    }
    model_evals[i]=X;
...
void DensityKOH::WritePredictions(vector<VectorXd> &vectorx,const char* file_name) const{
  //Ecriture de la prédiction moyenne et de l'incertitude associée, obtenue par méthode KOH. On fait un écart-type.
  ...
  {
    Contribf(i)=Varf.diagonal()(i)/(sqrt(Varf.diagonal()(i)+EDM.diagonal()(i)));
    ContribEDM(i)=EDM.diagonal()(i)/(sqrt(Varf.diagonal()(i)+EDM.diagonal()(i)));
  }
  FILE* out=fopen(file_name,"w");
...
MatrixXd DensityOpt::VarParamUncertaintyTotal(vector<VectorXd> &vectorx) const {
...
MatrixXd DensityOpt::VarParamUncertaintyZMean(vector<VectorXd> &vectorx) const {
  //Calcul de Var(f_theta) aux points du vectorx. Puisque cette variance est indépendante entre les points, on renvoie une matrice diagonale.
  //first : récupérer le vecteur d'évaluations du modèle aux points d'intérêt.
  int xsize=vectorx.size();
  //un premier calcul pour savoir combien de thetas ont une probabilité non nulle
  ...
  for (int i=0;i<xsize;i++)
  {
    ...
    for (int j=0;j<m_npts;j++){
      if(m_filtre[j]){
        vector<VectorXd> v(1);
        v[0]=vectorx[i];
        X(c)=MeanPredCondTheta(v,j)(0); //mauvais
        ++c;
      }
    }
    model_evals[i]=X;
...
bool MCMC::in_bounds(Eigen::VectorXd &X)
{
  ...
  for (int i=0;i<m_dim_hpars;i++){
    if (X(i+m_Grid.GetDimension())<m_lb_hpars[i] || X(i+m_Grid.GetDimension())>m_ub_hpars[i]) {++m_oob(i+m_dim_hpars); return false;}
  }
  return true;
}
