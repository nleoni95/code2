// Dans ce fichier : j'étudie la construction des hGPs.
// première étape : m'assurer que ça se passe bien, ensuite je ferai des tests de convergence selon le grid d'entraînement.
...
int main(int argc, char **argv){
...

  {
    ...
    
    {
      ...
      //calcul des hpars optimaux sur le grid_post, qui est celui contenu dans les samples chargés.
      DensOpt_alpha.ReadSamples("samples/sampoptalpha.gnu");
      DensOpt_diam.ReadSamples("samples/sampoptdiam.gnu");
      vector<VectorXd> grid_verif;
      grid_verif=DensOpt_alpha.GetSamples();
      vector<VectorXd> halpha_verif;
      vector<VectorXd> hdiam_verif;
      //test de vitesse. 20 optimisations.
      /*
      {
        vector<VectorXd> hpars;
        auto begin=chrono::steady_clock::now();
        for(int i=0;i<20;i++){
          hpars.push_back(DensOpt_diam.HparsOpt(grid_verif[i],hpars_z_guess_diam));
        }
        auto end=chrono::steady_clock::now();
        double ll_avg(0);
        for(int i=0;i<20;i++){
          ll_avg+=DensOpt_diam.loglikelihood_theta(grid_verif[i],hpars[i])+DensOpt_diam.EvaluateLogPHpars(hpars[i]);
        }
        cout << "20 optimisations à ftol 1e-3. temps : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << " s. avg ll : " << ll_avg/20 << endl;
        exit(0);
      }
      */
      ...
      bool fileverif=false;
      //lecture dans un fichier. des hpars de vérification.
      if(fileverif){
        grid_verif.clear();
        cout << "lecture des hpars verif dans un fichier..." << endl;
        ifstream ifile("results/hparsverif.gnu");
        if(ifile){
          string line;
          while(getline(ifile,line)){
            if (line[0]=='#'){continue;}
        ...
        cout << "hparsverif loaded : " << grid_verif.size() << endl;
      }
      else{
        grid_verif=DensOpt_alpha.GetSamples();
        auto begin_verif=chrono::steady_clock::now();
        for(int i=0;i<grid_verif.size();i++){
          VectorXd theta=grid_verif[i];
          if(i==0){
            halpha_verif.push_back(DensOpt_alpha.HparsOpt(theta,hpars_z_guess_alpha));
            hdiam_verif.push_back(DensOpt_diam.HparsOpt(theta,hpars_z_guess_diam));
          }
          else{
            halpha_verif.push_back(DensOpt_alpha.HparsOpt(theta,halpha_verif[i-1]));
            hdiam_verif.push_back(DensOpt_diam.HparsOpt(theta,hdiam_verif[i-1]));
          }
        }
        auto end_verif=chrono::steady_clock::now();
        cout << "temps de calcul hpars sur grid verif : " << chrono::duration_cast<chrono::seconds>(end_verif-begin_verif).count() << " s." << endl;
      }
      //remplissage en matrices
      MatrixXd hpars_verif_alpha(3,grid_verif.size());
      MatrixXd hpars_verif_diam(4,grid_verif.size());
      //écriture des hpars opti comme ça c'est fait.
      for(int i=0;i<grid_verif.size();i++){
        hpars_verif_alpha.col(i)=halpha_verif[i];
        hpars_verif_diam.col(i)=hdiam_verif[i];
      }
      ofstream ofile("results/hparsverif.gnu");
      for(int i=0;i<grid_verif.size();i++){
        VectorXd theta=grid_verif[i];
        for(int j=0;j<5;j++){
         ofile << theta(j) << " ";
        ...
        for(int j=0;j<3;j++){
          ofile << hpars_verif_alpha(j,i) << " ";
        }
        for(int j=0;j<4;j++){
          ofile << hpars_verif_diam(j,i) << " ";
        }
        ofile << endl;
      }
      ofile.close();
      ...
      //calcul des hyperparamètres optimaux sur un grid QMC de 2000 points. stockage dans un vecteur <AUGDATA>
      DoE doe_qmc(lb_t,ub_t,2000,1);
      vector<VectorXd> grid_qmc=doe_qmc.GetGrid();
      vector<AUGDATA> hparsopt_qmc_full_alpha;
      vector<AUGDATA> hparsopt_qmc_full_diam;
      //read les hpars qmc ?
      bool file_qmc=false;
      if(!file_qmc){
        auto begin_qmc=chrono::steady_clock::now();
        for(int i=0;i<grid_qmc.size();i++){
          VectorXd theta=grid_qmc[i];
          VectorXd hparsopt_alpha;
          VectorXd hparsopt_diam;
          if(i==0){
            hparsopt_alpha=DensOpt_alpha.HparsOpt(theta,hpars_z_guess_alpha);
            hparsopt_diam=DensOpt_diam.HparsOpt(theta,hpars_z_guess_diam);
          }
          else{
            hparsopt_alpha=DensOpt_alpha.HparsOpt_quick(theta,hparsopt_qmc_full_alpha[i-1].Value());
            hparsopt_diam=DensOpt_diam.HparsOpt_quick(theta,hparsopt_qmc_full_diam[i-1].Value());
          }
          AUGDATA dat; dat.SetX(theta),dat.SetValue(hparsopt_alpha);
          hparsopt_qmc_full_alpha.push_back(dat);
          dat.SetValue(hparsopt_diam);
          hparsopt_qmc_full_diam.push_back(dat);
        }
        auto end_qmc=chrono::steady_clock::now();
        cout << "temps de calcul hpars sur grid QMC : " << chrono::duration_cast<chrono::seconds>(end_qmc-begin_qmc).count() << " s." << endl;
        ofstream ofile3("results/hparsoptqmc.gnu");

        //écriture dans un fichier
        for(int i=0;i<hparsopt_qmc_full_alpha.size();i++){
          VectorXd x= hparsopt_qmc_full_alpha[i].GetX();
          VectorXd ha=hparsopt_qmc_full_alpha[i].Value();
          VectorXd hd=hparsopt_qmc_full_diam[i].Value();
          for(int i=0;i<x.size();i++){
            ofile3 << x(i) << " ";
          }
        ...
        cout << "lecture des hpars qmc dans un fichier..." << endl;
        ifstream ifile("results/hparsoptqmc.gnu");
        if(ifile){
          string line;
          while(getline(ifile,line)){
            if (line[0]=='#'){continue;}
            ...
            AUGDATA dat; dat.SetX(theta); dat.SetValue(hpars_alpha);
            hparsopt_qmc_full_alpha.push_back(dat);
            dat.SetValue(hpars_diam);
            hparsopt_qmc_full_diam.push_back(dat);
          }
        }
        cout << "hparsqmc loaded : " << hparsopt_qmc_full_alpha.size() << endl;
        //read les hpars qmc depuis un fichier
      }

      cout << "exemple de valeurs : " << hparsopt_qmc_full_alpha[0].GetX().transpose() << ", " << hparsopt_qmc_full_alpha[0].Value().transpose() << endl;
      cout << "exemple 2 : " << hparsopt_qmc_full_diam[1999].GetX().transpose() << ", " << hparsopt_qmc_full_diam[1999].Value().transpose() << endl;

      //fonction pour mesurer l'erreur de hGPs pour deux densopt déjà formées.
      auto evaluate_hgps=[&grid_verif,&halpha_verif,&hdiam_verif,&hpars_z_guess_diam,&hpars_z_guess_alpha]( DensityOpt & Dalpha, DensityOpt & Ddiam){
        vector<double> erralpha=Dalpha.Test_hGPs_on_likelihood(hpars_z_guess_alpha,halpha_verif);
        vector<double> errdiam=Ddiam.Test_hGPs_on_likelihood(hpars_z_guess_diam,hdiam_verif);
        VectorXd errors(9);
        errors<<erralpha[0],erralpha[1],erralpha[2],errdiam[0],errdiam[1],errdiam[2],errdiam[3],erralpha[3],errdiam[4];
        //erreurs sur les hpars puis sur la ll.
        return errors;
      };

      //création de deux densités opt avec un design de taille choisie.
      auto creation_densityopt=[&DensOpt_alpha,&DensOpt_diam,&lb_t,&ub_t,&hpars_gp_guess,&Bounds_hpars_gp,&hparsopt_qmc_full_alpha,&hparsopt_qmc_full_diam](int ngrid){
        vector<AUGDATA> new_hparsopt_alpha(ngrid);
        vector<AUGDATA> new_hparsopt_diam(ngrid);
        for(int i=0;i<ngrid;i++){
          new_hparsopt_alpha[i]=hparsopt_qmc_full_alpha[i];
          new_hparsopt_diam[i]=hparsopt_qmc_full_diam[i];
        }
        DensityOpt Dalpha=DensOpt_alpha;
        DensityOpt Ddiam=DensOpt_diam;
        auto begin=chrono::steady_clock::now();
        Dalpha.SetNewHparsOpt(new_hparsopt_alpha);
        Dalpha.BuildHGPs(Kernel_GP_SE,Bounds_hpars_gp,hpars_gp_guess,3);
        Dalpha.opti_allgps(hpars_gp_guess);
        Ddiam.SetNewHparsOpt(new_hparsopt_diam);
        Ddiam.BuildHGPs(Kernel_GP_SE,Bounds_hpars_gp,hpars_gp_guess,4);
        Ddiam.opti_allgps(hpars_gp_guess);
        auto end=chrono::steady_clock::now();
        double time=chrono::duration_cast<chrono::seconds>(end-begin).count();
        return make_tuple(Dalpha,Ddiam,time);
      };
      ofstream ofile2("results/hgps.gnu");
      ofile << "#6 premières colonnes : erreurs, col7 : temps d'évaluation des hgps sur le grid de validation, col8 : temps de construction + optimisation des hgps" << endl;

      for(int i=0;i<39;i++){
        int n=50+50*i;
        auto tuple_densities=creation_densityopt(n);
        VectorXd e=evaluate_hgps(get<0>(tuple_densities),get<1>(tuple_densities));
        ofile2 << n << " ";
        for(int i=0;i<e.size();i++){
          ofile2 << e(i) << " ";
        }
        ofile2 << get<2>(tuple_densities) << endl;
        cout << "erreurs pour n = " << n << " : " << e.transpose()<< " " << get<2>(tuple_densities) << endl << endl;
      }
      ofile2.close();
      exit(0);
    } 
