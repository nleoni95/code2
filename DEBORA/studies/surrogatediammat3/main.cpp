...
double Kernel_Z_Matern(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //squared exponential
  double d=abs(x(0)-y(0));
  return pow(hpar(0),2)*(1+d/hpar(2))*exp(-d/hpar(2));
...
int main(int argc, char **argv){
  ...
  //diam
  lb_hpars << 2e-5,1e-5,5e-4; 
  ub_hpars << 5e-4,3e-5,1e-1;
  ...
  COV_init_bayes(5,5)*=pow(0.1*(ub_hpars(0)-lb_hpars(0)),2); //sedm
  COV_init_bayes(6,6)*=pow(0.1*(ub_hpars(1)-lb_hpars(1)),2); //sobs
  COV_init_bayes(7,7)*=pow(0.1*(ub_hpars(2)-lb_hpars(2)),2); //lcor
  COV_init_bayes*=0.5;
  cout << "COV_init_bayes : " << endl << COV_init_bayes << endl;
  //construction du grid
  DoE doe_init(lb_t,ub_t,5);//2000,generator);//4);//5000,generator);//3125 pts 
  ...
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocorkohmat3.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampkohmat3.gnu");
  MainDensity.WriteSamples("results/diag/sampkohmat3.gnu");
  auto mcmc_koh=chrono::steady_clock::now();

  //MainDensity.ReadSamples("results/save/sampkohdiammat.gnu");
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamkohmat3F.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/diamkohmat3.gnu");
  MainDensity.WritePriorPredictionsF(VectorXd::Random(1),"results/preds/diampriormat3F.gnu",generator);
  ...
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamnoedmmat3F.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/diamnoedmmat3.gnu");
  MainDensity.WriteSamples("results/save/samplesnoedmdiammat3.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocornoedmmat3.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampnoedmmat3.gnu");
  


  cout << endl << "Début full bayes : " << endl;
  //full bayes
  MainDensity.Run_FullMCMC(nombre_steps_mcmc,nombre_samples_collected,COV_init_bayes,generator);
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/diamfullbayesmat3F.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/diamfullbayesmat3.gnu");
  MainDensity.WriteSamples("results/save/samplesfullbayesdiammat3.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocorfullbayesmat3.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampfullbayesmat3.gnu");
  ...
  */
  ...
  DensOpt.Initialise_GPs(Kernel_GP_SE,Bounds_hpars_gp,hpars_gp_guess);
  DensOpt.opti_allgps(hpars_gp_guess);
  DensOpt.Test_hGPs(generator);
  DensOpt.WritehGPs("results/save/hGPsdiammat3.gnu");
  //DensOpt.ReadhGPs("results/save/hGPsdiam.gnu"); //reprise des hpars sek
  ...
  VectorXd theta_middle(5);
  theta_middle << 0.91,0.70,0.45,0.48,0.66;
  VectorXd hpar_opt_thetamid=DensOpt.HparsOpt(theta_middle,hpars_z_guess);
  cout << "hpars thetamid : " << hpar_opt_thetamid.transpose()<< endl;
  VectorXd hpar_opt_thetamid2=DensOpt.EvaluateHparOpt(theta_middle);
  cout << "version gp : " << hpar_opt_thetamid2.transpose() << endl;
  double ll=DensOpt.loglikelihood_theta(*(DensOpt.GetExpData()),theta_middle,hpar_opt_thetamid);
  //afficher getexpdata pour être sûr.
  double lp=logprior_hpars(hpar_opt_thetamid);
  double ll2=DensOpt.loglikelihood_theta(*(DensOpt.GetExpData()),theta_middle,hpar_opt_thetamid2);
  double lp2=logprior_hpars(hpar_opt_thetamid2);
  cout << "ll : " << ll << " , lp : " << lp << endl;
  cout << "ll2 : " << ll2 << " , lp2 : " << lp2 << endl;
  DensOpt.WriteOneCalcul(VectorXd::Random(1),theta_middle,hpar_opt_thetamid,"results/thetamiddle.gnu");

  //on répète pour un sample normal de opti.

  VectorXd theta_samp(5);
  theta_samp << 0.92,0.70,0.34,0.49,0.60;
  VectorXd hpar_opt_thetasamp=DensOpt.HparsOpt(theta_samp,hpars_z_guess);
  cout << "hpars thetasamp : " << hpar_opt_thetasamp.transpose()<< endl;
  VectorXd hpar_opt_thetasamp2=DensOpt.EvaluateHparOpt(theta_samp);
  cout << "version gp : " << hpar_opt_thetasamp2.transpose() << endl;
  double llsamp=DensOpt.loglikelihood_theta(*(DensOpt.GetExpData()),theta_samp,hpar_opt_thetasamp);
  double lpsamp=logprior_hpars(hpar_opt_thetasamp);
  double llsamp2=DensOpt.loglikelihood_theta(*(DensOpt.GetExpData()),theta_samp,hpar_opt_thetasamp2);
  double lpsamp2=logprior_hpars(hpar_opt_thetasamp2);
  cout << "llsamp : " << llsamp << " , lpsamp : " << lpsamp << endl;
  cout << "llsamp2 : " << llsamp2 << " , lpsamp2 : " << lpsamp2 << endl;
  DensOpt.WriteOneCalcul(VectorXd::Random(1),theta_samp,hpar_opt_thetasamp,"results/thetasamp.gnu");

  auto gps_opt=chrono::steady_clock::now();


  DensOpt.Run_MCMC_opti(nombre_steps_mcmc,nombre_samples_collected,COV_init,generator);
  auto mcmc_opt=chrono::steady_clock::now();
  DensOpt.Autocor_diagnosis(nautocor,"results/diag/autocoroptmat3.gnu");
  DensOpt.WriteMCMCSamples("results/diag/allsampoptmat3.gnu");

  //DensOpt.ReadSamples("results/save/sampoptdiammat.gnu");

  DensOpt.WritePredictionsF(VectorXd::Random(1),"results/preds/diamoptmat3F.gnu");
  DensOpt.WritePredictions(VectorXd::Random(1),"results/preds/diamoptmat3.gnu");

  auto pred_opt=chrono::steady_clock::now();

  DensOpt.WriteSamples("results/save/sampoptdiammat3.gnu");
