...
/*
  for (int i:cases){
    ...
    int nmodel_evals=4000; //nombre d'évaluations du modèle pour faire le GP.
    ...
    auto lambda_model=[&pFunc_model,&pParamsRavik,i](double const &X, VectorXd const & theta) mutable{
    //renvoie toutes les prédictions du modèle aux points donnés par X.
      VectorXd x(1);
      x(0)=X;
      double pred=my_model(x,GPtoR(theta),i,pFunc_model,pParamsRavik);
      return pred;
    };
...
exit(0);
  ...
  {
    auto lambda_modeltest=[&pParamsRavik,&pFunc_model,&cases](VectorXd const &X, VectorXd const & theta) mutable{
      //renvoie toutes les prédictions du modèle aux points donnés par X.
        VectorXd pred(X.size());
        for(int j=0;j<X.size();j++){
          VectorXd x(1);
          x(0)=X(j);
          pred(j)=my_model(x,GPtoR(theta),cases[0],pFunc_model,pParamsRavik);
        }
        return pred;
      };
...
//début des calibrations.
  ...
  */
        ...
        //MCMC opt avec toutes les densités en même temps. on retente. ATTENTION PAS DE PREDICTIONS MOMENTANEMENT
...
//MCMC phase Bayes. sur la densité complète. oskour. 

  {
    ...
    for(int i=0;i<vsize;i++){
      ...
      COV_init_fb(3+i*m+1,3+i*m+1)=pow(2,2);
    ...
    auto in_bounds_fb=[&vDens,&vsize,&lb_hpars,&ub_hpars](VectorXd const & X){
      ...
      if(!vDens[0].in_bounds_pars(X)){in=false;}
      return in;
    };
    ...
    auto res=Run_MCMC_noburn(3*nombre_steps_mcmc,nombre_samples_collected,X_init_from_FMP,COV_init_from_FMP,compute_score_fb,get_hpars_fb,in_bounds_fb,generator);
...
for(int i=0;i<vsize;i++){
    auto in_bounds_fb=[&vDens,&vsize,&lb_hpars](VectorXd const & X){
      ...
      return vDens[0].in_bounds_hpars(hp);
    };
...
//MCMC kohseparate toutes les expériences
  ...
  //cout << hparskoh_separate[1].transpose() << endl;
  {
    ...
    auto res=Run_MCMC(nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc,COV_init,compute_score_kohs,get_hpars_kohs,in_bounds,generator);
      ...
      //diagnostic
      vDens[0].Autocor_diagnosis(nautocor,"results/fulldensities/koh/autocorkohs.gnu");
      //écriture des samples. Il faut une fonction dédiée. 
      for(int i=0;i<vsize;i++){
        auto lambda_modeltrue=[&pFunc_model,&pParamsRavik,&cases,i](VectorXd const &X, VectorXd const & theta) mutable{
        //renvoie toutes les prédictions du modèle aux points donnés par X.
