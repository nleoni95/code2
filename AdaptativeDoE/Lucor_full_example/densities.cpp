...
VectorXd DensityKOH::HparsKOH() {
...

  {
    ...
    hparstest[2]=25;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
    hparstest[0]=177699.8;
    hparstest[1]=64565.35;
    hparstest[2]=11.17;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
    hparstest[0]=100000;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
...
VectorXd MCMC_opti::Opti_hpars(vector<DATA> &data_quelconque,VectorXd &hpars_precedent){
  //optimisation des hyperparamètres pour un vecteur de données quelconque. Bien vérifier que les X du vecteur de données coincident avec les X des observations
  //on n'a pas retranché les observations à data_quelconque !
  //création des nouvelles données
  std::default_random_engine generator;
  std::uniform_real_distribution<double> distU(0,1);
  int maxeval=5000;
  ...
  //construction de la densité opt. On fait une boucle sur les theta.
	  ...
	  for(int ie=0; ie<nd; ie++){
	    DATA dat;
      dat.SetX(data_quelconque[ie].GetX());
      if(!(data_quelconque[ie].GetX()==GetObs()[ie].GetX())){cout << "erreur. data_quelconque X :" << data_quelconque[ie].GetX().transpose() << " et les obs : " << GetObs()[ie].GetX().transpose() << endl;};
      dat.SetValue((data_quelconque[ie].Value()));//observations moins évaluations du modèle. on ne met pas prior mean car c'est fait dans optfunc.
      data2.push_back(dat); // on construit les données y-f_t      
    }
    //on construit la structure AugmentedMCMC_opti
    AugmentedMCMC_opti AM(this,&data2);
