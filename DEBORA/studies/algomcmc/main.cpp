...
// J'essaye de montrer l'avantage d'utiliser mon petit procédé de correction de la matrice de covariance.
// On fait un Full Bayes avec une phase de burn, 1 avec ma correction et 1 sans. 
...
int main(int argc, char **argv){
  ...
  DoE doe_init(lb_t,ub_t,3125,generator);//2000,generator);//4);//5000,generator);//3125 pts 
  ...
  //full bayes sans mon algo
  MainDensity.Run_FullMCMC(nombre_steps_mcmc,nombre_samples_collected,COV_init_bayes,generator);
  MainDensity.WriteSamples("results/save/samplesfb.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocorfb.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampfb.gnu");
  cout << "evidence without scaling : " << MainDensity.Evidence(30);

  //full bayes avec mon algo scaling
  MainDensity.Run_FullMCMCWithScale(nombre_steps_mcmc,nombre_samples_collected,COV_init_bayes,generator);
  MainDensity.WriteSamples("results/save/samplesfbscaling.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocorfbscaling.gnu");
  MainDensity.WriteMCMCSamples("results/diag/allsampfbscaling.gnu");
  cout << "evidence with scaling : " << MainDensity.Evidence(30);
