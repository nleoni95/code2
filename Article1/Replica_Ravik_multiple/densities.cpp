...
void Run_Burn_Phase_MCMC(int nburn, MatrixXd &COV_init, VectorXd &Xcurrento, function<double(vector<VectorXd>, VectorXd const &)> const &compute_score, function<vector<VectorXd>(VectorXd const &)> const &get_hpars, function<bool(VectorXd)> const &in_bounds, default_random_engine &generator)
{
  ...
  double oob_rate = (double)(noob) / (double)(nburn);
...
void Density::SetInputerr(bool b, double value, int index, VectorXd derivatives_at_obs, VectorXd derivatives_at_preds)
{
...
double Density::GetInputerr(VectorXd const &hpars) const
{
  if (m_inputerr == -1.0)
  {
    //cout << "inputerr learned:" << hpars(m_indexinputerr) << endl;
    //inputerr is learned
...
{
  //Il FAUT que X coïncide avec les Xlovations données lors de l'appel à la fct inputerr. Sinon ça rendra une erreur.
...
vector<VectorXd> Density::Draw_likelihood_samples(VectorXd const &theta, VectorXd const &hpars, int nsamples, default_random_engine &generator) const
{
  //tirage d'échantilons de la likelihood. Aux points d'observations.
  int nobs = m_Xlocations.size();
  VectorXd mean = m_model(m_Xlocations, theta);
  VectorXd C = Gamma(m_Xlocations, hpars) + pow(GetOutputerr(hpars), 2) * MatrixXd::Identity(nobs, nobs) + pow(GetInputerr(hpars), 2) * GetDerMatrix();
  MatrixXd sqrtC = C.llt().matrixL();
  normal_distribution<double> distN(0, 1);
  ...
  for (int i = 0; i < nsamples; i++)
  {
    ...
    {
      Seed(j) = distN(generator);
    }
    VectorXd Samp=mean+sqrtC*Seed;
    samples.push_back(Samp);
  }
  return samples;
}

double Density::Compute_reference_prior(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars, int nsamples_ll, int niter,default_random_engine & generator) const{
  //calcule le reference prior pour une valeur (pars,hpars) avec uniform reference prior (see Berger & Bernardo 2007)
  for(int i=0;i<niter;i++){
    vector<VectorXd> samp=Draw_likelihood_samples(theta,hpars,nsamples_ll,generator);
...
//dérivées par méthodes parce que sinon ça marche pas...
