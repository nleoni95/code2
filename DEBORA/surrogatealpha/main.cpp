...
int main(int argc, char **argv){
  ...
  */
  ...
  //tdv
  lb_hpars << 1e-4,1e-4,1e-4;
  ub_hpars << 1,1e-1,1e-1;
  ...
  MainDensity.SetKernel(Kernel_Z_Matern32);
  MainDensity.SetKernelDerivatives(D1Kernel_Z_Matern32,D2Kernel_Z_Matern32,D3Kernel_Z_Matern32);
...
/*
  ...
  cout << "temps total construction hgps : " << chrono::duration_cast<chrono::seconds>(gps_opt-begin_opt).count() << " s" << endl << endl;
  cout << "vérification des hgps..." << endl;
  DensOpt.Test_hGPs();
...
*/
  ...
  MainDensity.Run_FullMCMC(10*nombre_steps_mcmc,nombre_samples_collected,Xinit_bayes,COV_init_bayes,generator);
  ...
  MainDensity.Run_MCMC_fixed_hpars(nombre_steps_mcmc,nombre_samples_collected,0.1*COV_init,hpars_noedm,generator);
  MainDensity.WritePredictionsF(VectorXd::Random(1),"results/preds/alphanoedmF.gnu");
  MainDensity.WritePredictions(VectorXd::Random(1),"results/preds/alphanoedm.gnu");
  MainDensity.WriteFinePredictions(VectorXd::Random(1),"results/preds/alphanoedmfine.gnu");
  MainDensity.WriteSamples("results/save/sampalphanoedm.gnu");
  MainDensity.Autocor_diagnosis(nautocor,"results/diag/autocornoedm.gnu");
