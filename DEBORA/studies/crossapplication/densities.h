...
class Density
{
    ...
    int optroutine(nlopt::vfunc optfunc,void *data_ptr, Eigen::VectorXd &x, Eigen::VectorXd const & lb_hpars, Eigen::VectorXd const & ub_hpars, int max_time) const;
    int optroutine_withgrad(nlopt::vfunc optfunc,void *data_ptr, Eigen::VectorXd &x, Eigen::VectorXd const & lb_hpars, Eigen::VectorXd const & ub_hpars, int max_time)const;
    ...
    Eigen::VectorXd HparsKOH(Eigen::VectorXd const & hpars_guess, int max_time) const;
    ...
    Eigen::VectorXd HparsLOOCV(Eigen::VectorXd const & hpars_guess, int max_time) const;
...
class DensityOpt : public Density{
    ...
    Eigen::VectorXd HparsOpt(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars_guess,int max_time)const;
    Eigen::VectorXd HparsOpt_quick(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars_guess,int max_time)const;
    ...
    void Compute_optimal_hpars(int max_time);
    ...
    void Test_hGPs(int npoints, int max_time);
    ...
    void update_hGPs(std::vector<Eigen::VectorXd> const &new_thetas,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &),Eigen::MatrixXd const & Bounds_hpars_GPs,Eigen::VectorXd const & Hpars_guess_GPs,int nmodes, int max_time);

    //Fonctions de MCMC

    double Run_Burn_phase_MCMC_opti_expensive(int nburn, Eigen::MatrixXd & COV_init,Eigen::VectorXd &Xcurrent,std::default_random_engine & generator, int max_time);
    void Run_MCMC_opti_expensive(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator, int max_time);
...
class Densities{
    ...
    int optroutine(nlopt::vfunc optfunc,void *data_ptr, std::vector<double> &x, std::vector<double> const & lb_hpars, std::vector<double> const & ub_hpars,int max_time);
    ...
    std::vector<Eigen::VectorXd> HparsKOH_separate(std::vector<Eigen::VectorXd> const & hpars_guess_vec, int max_time);
    std::vector<Eigen::VectorXd> HparsKOH_pooled(std::vector<Eigen::VectorXd> const & hpars_guess_vec, int max_time);
...
class DensitiesOpt : public Densities{
    ...
    void compute_optimal_hpars(int max_time);
    std::vector<Eigen::VectorXd> HparsOpt(Eigen::VectorXd const & theta,std::vector<Eigen::VectorXd> const & hpars_guess_vec, int max_time) const;
