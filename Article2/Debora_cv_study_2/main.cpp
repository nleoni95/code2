...
// ce fichier est une copie de Debora_cv_study, mais la différence est que l'algorithme update à chaque fois que l'on rajoute un point les poids des différents candidats.
...
{
  //n'oublions pas que t sont dans [0,1].
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  /*Algorithme de sélection automatique de points. n50*/
  {
    ...
    auto add_points = [&Doalpha, &Dodiam, npts_per_iter, nsteps_mcmc, nsamples_mcmc, &X_init_mcmc, &COV_init, &generator, hpars_z_guess_diam, hpars_z_guess_alpha, &thetas_training, &halpha_training, &hdiam_training, time_opti_fine, &Bounds_hpars_hgps]()
    {
      ...
      auto hpars_opt_hgps_alpha = Doalpha.GetHparsHGPs();
      auto hpars_opt_hgps_diam = Dodiam.GetHparsHGPs();
      ...
      for (int i = 0; i < npts_per_iter; i++)
      {
        ...
        for (int j = 0; j < weights.size(); j++)
        {
          ...
          {
            weights[j] = Doalpha.EstimatePredError(candidate_thetas[j]) + Dodiam.EstimatePredError(candidate_thetas[j]);
          }
        }
        ...
        VectorXd halpha = Doalpha.HparsOpt(selected_thetas[i], hpars_z_guess_alpha, time_opti_fine);
        VectorXd hdiam = Dodiam.HparsOpt(selected_thetas[i], hpars_z_guess_diam, time_opti_fine);
        ...
        Doalpha.SetHGPs(hpars_opt_hgps_alpha);
        Dodiam.SetHGPs(hpars_opt_hgps_diam);
      }
      // on remet cout.
      cout.clear();
    ...
    while (thetas_training.size() <= 1500)
    {
  ...
  /*Test : je rajoute progressivment des points du true sample dans le training set. Attention. Je ne dois aps rajouter les mêmes points mais plutôt des points tirés de la même densité... subtil...*/
  {
    ...
    for (auto const &v : valstrain)
    {
      ...
      VectorXd halpha(3);
      halpha << v(5), v(6), v(7);
      thetas_training_reserve.push_back(theta);
      hdiam_training_reserve.push_back(hdiam);
      halpha_training_reserve.push_back(halpha);
    }
    //compute scores for the true FMP sample
    vector<double> scores_true_alpha;
    vector<double> scores_true_diam;
    ...
    //lambda fonction pour rajouter un certain nombre de points et calculer les hpars optimaux.
    auto add_points = [&Doalpha, &Dodiam, npts_per_iter, &thetas_training_reserve, &halpha_training_reserve, &hdiam_training_reserve, &thetas_training, &halpha_training, &hdiam_training]()
    {
      //juste rajouter npts_per_iter points depuis le thetas_training_reserve
      int npts = thetas_training.size();
      for (int i = 0; i < npts_per_iter; i++)
      {
        if (npts + i >= thetas_training_reserve.size())
        {
          break;
        }
        thetas_training.push_back(thetas_training_reserve[npts + i]);
        halpha_training.push_back(halpha_training_reserve[npts + i]);
        hdiam_training.push_back(hdiam_training_reserve[npts + i]);
      }
    };
