...
double Kernel_Z_LS(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //squared exponential
  return 0;
}

double D1Kernel_Z_LS(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //dérivée par rapport à sigma
  return 0;
}

double D2Kernel_Z_LS(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  return 0;
}

double D3Kernel_Z_LS(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  return 0;
}
...
VectorXd HparsKOH_double(VectorXd const & hpars_guess,VectorXd const & lb_hpars,VectorXd const & ub_hpars,Density const & D1, Density const & D2,vector<AUGDATA> const & data_exp_1,vector<AUGDATA> const & data_exp_2){
  ...
  guess=optroutine(optfuncKOH_Double,&tp,guess,lb_hpars,ub_hpars,60); //60 secondes car ça ne change rien...
  return guess;
}
...
int main(int argc, char **argv){
...

  {
    ...
    MainDensity_alpha.SetKernel(Kernel_Z_LS);MainDensity_alpha.SetKernelDerivatives(D1Kernel_Z_LS,D2Kernel_Z_LS,D3Kernel_Z_LS);
    ...
    MainDensity_diam.SetKernel(Kernel_Z_LS);MainDensity_diam.SetKernelDerivatives(D1Kernel_Z_LS,D2Kernel_Z_LS,D3Kernel_Z_LS);
    ...
    cout << "on impose le rapport des bruits pour équilibrer les likelihood :" << endl;
    hparskohdouble(1)=1e-2; //sigma alpha
    hparskohdouble(4)=2e-5; //sigma diam
