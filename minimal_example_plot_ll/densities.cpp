...
double Density::Run_Burn_phase_MCMC_fixed_hpars(int nburn, MatrixXd & COV_init,VectorXd const & hpars, VectorXd & Xcurrento, default_random_engine & generator) {
...
void Density::Run_MCMC_fixed_hpars(int nsteps,int nsamples, VectorXd & Xinit, MatrixXd const & COV_init,VectorXd const & hpars,default_random_engine & generator){
  ...
  Run_Burn_phase_MCMC_fixed_hpars(nsteps*0.1,COV,hpars,Xinit,generator);
...
void Density::Autocor_diagnosis(int nstepsmax, string const & filename) const
{
  ...
  //on oublie les 10% premières steps dans le diagnostic.
...
double DensityOpt::optfuncOpt(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  //n'utilise pas le gradient. cette fonction a été remplacée par optfuncopt_withgrad.
...
void DensityOpt::Build_hGPs(double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes){
