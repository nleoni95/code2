...
class Density
{
    ...
    const std::vector<Eigen::VectorXd> GetSamples() const{return m_samples;}
...
class DensityOpt : public Density{
    ...
    std::vector<double> Test_hGPs_on_likelihood(Eigen::VectorXd const & hpars_guess,std::vector<Eigen::VectorXd> const & true_hpars_sample);
    void SetNewHparsOpt(const std::vector<AUGDATA> &v){m_hpars_opti=v;}
