...
Density::Density(Density const &d) : m_Kernel(d.m_Kernel),m_logpriorhpars(d.m_logpriorhpars),m_logpriorpars(d.m_logpriorpars),m_my_model(d.m_my_model),m_priormean(d.m_priormean)
{
  m_npts=d.m_npts;
  m_Grid=d.m_Grid;
...
DensityKOH::DensityKOH(Density const &d, int c) : Density(d){
  m_norm_cst=c;
}
...
double Density::optfunc_d(const std::vector<double> &x, std::vector<double> &grad, void *data){
  pair<vector<DATA>,const Density*>* ext_info=(pair<vector<DATA>,const Density*>*) data; // récupération par pointeur
  const Density *D=ext_info->second;
  vector<DATA> newobs=ext_info->first;
...
VectorXd Density::Opti_hpars(VectorXd &theta,VectorXd &hpars_guess) const{
  ...
  //construction de la densité opt. On fait une boucle sur les theta.
      ...
      dat.SetValue((GetObs()[ie].Value())-m_my_model(GetObs()[ie].GetX(),theta));//observations moins évaluations du modèle. on ne met pas prior mean car c'est fait dans optfunc.
      data2.push_back(dat); // on construit les données y-f_t      
    }
    //on construit la structure AugmentedMCMC_opti
    pair<vector<DATA>,const Density*> info=make_pair(data2,this);
    ...
    for (int j=0;j<m_dim_hpars;j++){
        x[j]=hpars_guess(j); //warm_restart
    } 
    // initial guess}
    nlopt::opt opt(nlopt::GN_ISRES, m_dim_hpars);    /* algorithm and dimensionality */
	  opt.set_max_objective(Density::optfunc_d, &info);
	  opt.set_lower_bounds(m_lb_hpars);
	  opt.set_upper_bounds(m_ub_hpars);
	  opt.set_maxeval(maxeval);
	  opt.set_population(pop);
...
DensityBayes::DensityBayes(Density const &d) : Density(d){
...
DensityCV::DensityCV(Density const &d) : DensityKOH(d,0)
{
...
DensitySimple::DensitySimple(Density const &d, double c) : DensityKOH(d,c)
{
...
MCMC::MCMC(Density const &d, int nchain) : Density(d)
{
  m_nchain=nchain;
...
double MCMC_opti::optfunc(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  //maintenant qu'on connaît les hyperparamètres, il faut soustraire la moyenne a priori aux données. /!
  for(int i=0;i<newobs.size();i++){
    newobs[i].SetValue(newobs[i].Value()-M->m_priormean(M->GetObs()[i].GetX(),hpars));
  }
