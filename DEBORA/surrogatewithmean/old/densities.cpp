...
void DoE::Fill(std::function<Eigen::VectorXd(Eigen::VectorXd const &)> my_model,vector<DATA> *obs)
{
...
void DoE::Fill_Predictions(vector<double> &vectorx)
{
  m_vectorx=vectorx;
  //On évalue le modèle sur chaque point dont on fera la prédiction.
  //vectorx doit impérativement correspondre aux x du profil. Pour être certain de ne pas confondre, on le met en vecteur double.
  clock_t c_start = std::clock();
  ...
  m_model_evals_predictions=m_model_evals;
  clock_t c_end = std::clock();
  double time_elapsed_ms = 1000.0 * (c_end-c_start) / CLOCKS_PER_SEC;
  double time_exec_doe=time_elapsed_ms / 1000.0;
...
Density::Density(DoE g) : m_Grid(g),m_npts(g.GetGrid().size()),m_dim_pars(g.GetDimension()),m_model_evals(g.GetModelEvals()),m_obs(g.GetObs()),m_vectorx(g.GetVectorX()), m_model_evals_predictions(g.GetModelPredEvals())
{
...
Eigen::MatrixXd Density::Gamma(void const *data, Eigen::VectorXd const &hpar) const {
  ...
  for(int i=0; i<nd; i++){
    for(int j=i; j<nd; j++){
      A(i,j) = m_Kernel((*data2)[i].GetX()(0),(*data2)[j].GetX()(0), hpar);
      if(i!=j){
	A(j,i) = A(i,j);
      }else{
...
double Density::loglikelihood_theta_fast(int d, Eigen::VectorXd const &hpar, LDLT<MatrixXd> const &ldlt)const{
  ...
  for (int i=0;i<nd;i++){obs_theta(i)=m_obs[i].Value()-(m_model_evals)[d](i)-m_priormean(m_obs[i].GetX()(0),hpar);}
...
double Density::loglikelihood_theta(int d, Eigen::VectorXd const &hpar)const{
  ...
  for(unsigned ie=0; ie<nd; ie++){
    DATA dat; dat.SetX(m_obs[ie].GetX()); dat.SetValue(m_obs[ie].Value()-(m_model_evals)[d](ie)-m_priormean(m_obs[ie].GetX()(0),hpar));
    data3.push_back(dat);
  }
...
double DensityKOH::optfunc(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  obs=&observations;
  vector<VectorXd> Gridou=D->m_Grid.GetGrid(); //on récupère les theta
  ...
  for (int i=0;i<npts;i++){
    ...
    avg+=ok*exp(D->m_logpriorpars(Gridou[i]));
  }

  avg=avg*exp(D->m_logpriorhpars(hpar));
  //cout << "para testé : " << x[0] << "," << x[1] << ", " << x[2] << ", critère : " << avg << endl;
  return avg;
}
...
VectorXd DensityKOH::HparsKOH() {
  ...
  int maxeval=5000;
  double ftol=1e-4; //10% de tolérance
  ...
  /*Pointer to member*/
  {
    ...
    //2ème étape : optimisation locale par sbplx en partant du point trouvé précédemment.
    {
      ...
      int niter_optimisations=200; // On recommence l'opti tant qu'on n'a pas stagné 100 fois.
      int nopt=0;
  ...
  {
    //boucle de test pour d'autres hyperparamètres de KOH.
    vector<double> gradtest;
    vector<double> hparstest(3);
    hparstest[0]=250011;
    hparstest[1]=68078;
    hparstest[2]=12.12;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
    hparstest[2]=12.8;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
    hparstest[2]=12.5;
    cout << hparstest[0] << " " << hparstest[1] << " " << hparstest[2] << " : critère " << optfunc(hparstest,gradtest,this) << endl;
...
VectorXd DensityKOH::ZMeanCondTheta(int t) const{
  ...
  for (int i=0;i<y.size();i++){
    y(i)=m_obs[i].Value()-((m_model_evals)[t](i)+m_priormean(m_obs[i].GetX()(0),Hparscourant));
  }
  MatrixXd Kstar(m_vectorx.size(),m_obs.size());
  for (int i=0;i<m_vectorx.size();i++){
    for (int j=0;j<m_obs.size();j++){
      Kstar(i,j)=m_Kernel(m_vectorx[i],m_obs[j].GetX()(0),Hparscourant);
    }
  }
...
MatrixXd DensityKOH::VarZCondTheta(int t) const{
  //renvoie la variance de z a posteriori given theta. Ne dépend pas de theta dans le cas koh.
  VectorXd Hparscourant=m_hpars; //valeur des hpars
  MatrixXd Kstar1(m_vectorx.size(),m_obs.size());
  MatrixXd Kstar2(m_obs.size(),m_vectorx.size());
  for (int i=0;i<m_vectorx.size();i++){
    for (int j=0;j<m_obs.size();j++){
      Kstar1(i,j)=m_Kernel(m_vectorx[i],m_obs[j].GetX()(0),Hparscourant);
      Kstar2(j,i)=m_Kernel(m_obs[j].GetX()(0),m_vectorx[i],Hparscourant);
    }
  }
...
MatrixXd DensityKOH::VarFZ() const{
...
void DensityKOH::WritePredictions(const char* file_name) const{
  ...
  for (int i=0;i<Mean.size();i++)
  {
    fprintf(out, "%e %e %e %e\n",m_vectorx[i],Mean(i),Contribf(i),ContribEDM(i));
  }
  fclose(out);
}
...
void DensityKOH::WritePredictionsFZ(const char* file_name) const{
  ...
  for (int i=0;i<MeanF.size();i++)
  {
    fprintf(out, "%e %e %e %e\n",m_vectorx[i],MeanF(i),Contribf(i),MeanZ(i));
  }
  fclose(out);
}
...
void DensityOpt::WritePredictionsFZ(const char* file_name) const{
  ...
  VectorXd MeanF=EspF();
  VectorXd MeanZ=EspZ();
  MatrixXd Varf=VarF(); //variance de f_theta: "erreur de prédiction due à l'incertitude paramétrique"
  //on écrit dans le fichier directement les prédictions (et non les écarts)
  VectorXd Contribf(MeanF.size());
  for (int i=0;i<MeanF.size();i++)
  {
...
double DensityOpt::optfunc(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  for(int i=0;i<newobs.size();i++){
    newobs[i].SetValue(newobs[i].Value()-D->m_priormean(D->GetObs()[i].GetX()(0),hpars));
  }
...
VectorXd DensitySimple::HparsSimple() {
  ...
  /*Pointer to member*/
  {
    ...
    //2ème étape : optimisation locale par sbplx en partant du point trouvé précédemment.
    {
      ...
      msuploc=msup;
      cout << "Message d'arrêt : " << fin  << ". Max : " << hpars_max_koh.transpose() << ", valeur du critère : " << msuploc << endl;
      int niter_optimisations=50; // On recommence l'opti tant qu'on n'a pas stagné 2000 fois.
      int nopt=0;
      while (nopt<niter_optimisations){
	nopt++;
	x=RandHpars(generator);
...
double MCMC::loglikelihood_theta(void *data, Eigen::VectorXd const &hpar, VectorXd const &theta)const{
  ...
  int nd=data2->size();
  std::vector<DATA> data3;
  VectorXd ModelPred(nd); //nd est nrayons
  ModelPred=m_my_model(theta);
  for(unsigned ie=0; ie<nd; ie++){
    DATA dat; dat.SetX((*data2)[ie].GetX()); dat.SetValue((*data2)[ie].Value()-ModelPred(ie)+m_priormean((*data2)[ie].GetX()(0),hpar));
    data3.push_back(dat);
  }
  return loglikelihood(&data3,hpar);
}
...
VectorXd MCMC::ZMeanCondX(int t) const{
  ...
  VectorXd Zmean(m_vectorx.size());
  VectorXd y(m_obs.size());
  VectorXd Fpred=m_my_model(Thetacourant);
  for (int i=0;i<y.size();i++){
    y(i)=m_obs[i].Value()-Fpred(i)+m_priormean(m_obs[i].GetX()(0),Hparscourant);
  }
  MatrixXd Kstar(m_vectorx.size(),m_obs.size());
  for (int i=0;i<m_vectorx.size();i++){
    for (int j=0;j<m_obs.size();j++){
...
void MCMC::Autocorrelation_diagnosis(int n)
{
  ...
  for (int i=0;i<n;i++){
    ...
    fprintf(out, "%e %e %e %e %e %e\n",cor(0),cor(1),cor(2),cor(3),cor(4),cor(5));
  }
  fclose(out);
}

Eigen::VectorXd MCMC::Lagged_mean(int n) const{
...
MCMC_opti::MCMC_opti(MCMC &m, int nopti) : MCMC(m){
  m_nopti=nopti;
}

double MCMC_opti::optfunc(const std::vector<double> &x, std::vector<double> &grad, void *data){
  AugmentedMCMC_opti* AM = (AugmentedMCMC_opti*) data; //récupération de l'argument par pointeur
  MCMC_opti *M=AM->M;
  std::vector<DATA> newobs=*(AM->newobs); //recopie des newobs ici.
  Eigen::VectorXd hpars(M->m_dim_hpars);
  for(int p=0; p<M->m_dim_hpars; p++) {hpars(p) =x[p];} // copie des hyperparamètres dans un VectorXd
  //maintenant qu'on connaît les hyperparamètres, il faut soustraire la moyenne a priori aux données. /!
  for(int i=0;i<newobs.size();i++){
    newobs[i].SetValue(newobs[i].Value()-M->m_priormean(M->GetObs()[i].GetX()(0),hpars));
  }
  double d=M->loglikelihood(&newobs,hpars)+M->LogPriorHpars(hpars);
  return d;
}
//et il faut pouvoir évaluer la vraisemblance aussi. à un theta quelconque, donc ça demande une évaluation du modèle.

VectorXd MCMC_opti::Opti_hpars(VectorXd &theta,VectorXd &hpars_precedent) {
  //optimisation des hyperparamètres sur la valeur de theta courante.
  //création des nouvelles données
  std::default_random_engine generator;
  std::uniform_real_distribution<double> distU(0,1);
  int maxeval=5000;
  double ftol=1e-4;
  int nd=GetObs().size(); //nombre d'observations
  //construction de la densité opt. On fait une boucle sur les theta.
       	//Creation des data
	  std::vector<DATA> data2;
    VectorXd Fpred=m_my_model(theta);
	  for(int ie=0; ie<nd; ie++){
	    DATA dat; 
      dat.SetX(GetObs()[ie].GetX());
      dat.SetValue((GetObs()[ie].Value())-Fpred(ie));//observations moins évaluations du modèle. on ne met pas prior mean car c'est fait dans optfunc.
      data2.push_back(dat); // on construit les données y-f_t      
    }
    //on construit la structure AugmentedMCMC_opti
    AugmentedMCMC_opti AM(this,&data2);
    AugmentedMCMC_opti* AMpt=&AM;
    VectorXd hpars_max_opti(m_dim_hpars);
    {
	  int pop=2000; // population size
	  //	cout << "Début de l'optimisation globale... Pop size : " << pop << endl;
    ...
    // initial guess}
    nlopt::opt opt(nlopt::GN_ISRES, m_dim_hpars);    /* algorithm and dimensionality */
	  opt.set_max_objective(MCMC_opti::optfunc, AMpt);
	  opt.set_lower_bounds(m_lb_hpars);
	  opt.set_upper_bounds(m_ub_hpars);
	  opt.set_maxeval(maxeval);
	  opt.set_population(pop);
...
void MCMC_opti::Run(Eigen::VectorXd &Xinit, Eigen::MatrixXd &COV_init,default_random_engine &generator)
{
  std::normal_distribution<double> distN(0,1);
  std::uniform_real_distribution<double> distU(0,1);
  MatrixXd sqrtCOV=COV_init.llt().matrixL();
  cout << "Running MCMC_opti with " << m_nchain << " steps, and opti rate 1/" << m_nopti << endl;
  if(!COV_init.cols()==m_dim_mcmc){cout <<"erreur de dimension MCMC" << endl;}
  VectorXd Xcurrent=Xinit;
  VectorXd Xcandidate(m_dim_mcmc);
  double fcurrent=loglikelihood_theta(&m_obs,Xcurrent.tail(m_dim_hpars),Xcurrent.head(m_dim_pars))+m_logpriorhpars(Xcurrent.tail(m_dim_hpars))+m_logpriorpars(Xcurrent.head(m_dim_pars));
  ...
  clock_t c_start = std::clock();
  for (int i=0;i<m_nchain;i++){
    //optimisation des hpars si il est temps de le faire
    if(i%(m_nopti)==0){
      VectorXd theta=Xcurrent.head(m_dim_pars);
      VectorXd hpars=Xcurrent.tail(m_dim_hpars);
      Xcurrent.tail(m_dim_hpars)=Opti_hpars(theta,hpars);
    }

    //sinon : proposal mais en ne modifiant pas les hpars.
    //toute la partie basse de la matrice de covariance est inutile.

    VectorXd Step(m_dim_mcmc);
    for (int j=0;j<Step.size();j++){Step[j]=distN(generator);}
    Xcandidate=Xcurrent+sqrtCOV*Step;
    Xcandidate.tail(m_dim_hpars)=Xcurrent.tail(m_dim_hpars);
    fcandidate=loglikelihood_theta(&m_obs,Xcandidate.tail(m_dim_hpars),Xcandidate.head(m_dim_pars))+m_logpriorhpars(Xcandidate.tail(m_dim_hpars))+m_logpriorpars(Xcandidate.head(m_dim_pars));
    if (in_bounds(Xcandidate)){
      if(fcandidate>fcurrent || fcandidate-fcurrent>log(distU(generator))){
        m_naccept++;
  ...
  double time_elapsed_ms = 1000.0 * (c_end-c_start) / CLOCKS_PER_SEC;
  double time_mcmc=time_elapsed_ms / 1000.0;
  std::cout << "Temps pour MCMC_opti : " << time_mcmc << " s\n";
  cout << "accept rate : " << 100*double(m_naccept)/double(m_nchain) << endl;
}