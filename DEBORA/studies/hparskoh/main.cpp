//Etude de convergence des hpars koh.
//On fait des QMC de taille de plus en plus grande en calculant les hpars koh pour chacun.
...
int main(int argc, char **argv){
  ...
  */
  ...
  {
    cout << endl << "Début étude KOH" << endl;
  ...
  //répétitions des grid
  //liste des starting indices
  list<int> l={1,1000,2000,3000,4000,5000,6000,7000};

  for(int i:l){
    int nstart_qmc=i;
    cout << "début calcul grid " << i << endl;
    string name=to_string(nstart_qmc);
    DoE doe_init(lb_t,ub_t,1,nstart_qmc);
    ...
    VectorXd hpars_try=0.5*(lb_hpars+ub_hpars);

    int nmin=5; //on commence avec un QMC de 50 points
    int nincrement=5; //on rajoute 50 points à chaque fois
    int nsteps=100; //40 optimisations
    vector<int> all_npts(nsteps);
    vector<VectorXd> all_hpars(nsteps);
    vector<double> all_times(nsteps);
    ofstream ofile("results/results"+name+".gnu");
    for(int i=0;i<nsteps;i++){
      int npts=nmin+i*nincrement;
      DoE new_doe(lb_t,ub_t,npts,nstart_qmc);
      string gridname="results/grids/"+name+"grid"+to_string(npts)+".gnu";
      new_doe.WriteGrid(gridname);
      MainDensity.SetNewDoE(new_doe);
      auto begin=chrono::steady_clock::now();
      VectorXd hpars=MainDensity.HparsKOH(hpars_try);
      auto end=chrono::steady_clock::now();
      double time=chrono::duration_cast<chrono::seconds>(end-begin).count();
        all_npts[i]=npts;
        all_hpars[i]=hpars;
        all_times[i]=time;
        ofile << all_npts[i] << " " << all_hpars[i](0) << " " << all_hpars[i](1) << " "<< all_hpars[i](2) << " " << all_times[i] << endl;
    }
    ofile.close();  
  }  
}
