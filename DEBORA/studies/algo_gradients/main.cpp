...
int main(int argc, char **argv){
  ...
  cout << "début Opt. Comparaison des algorithmes pour optimisation des hpars sur 500 pts :" << endl;

  DensityOpt DensOpt(MainDensity);
  //cout << "LN_SBPLX" << endl;
  auto sbplx=chrono::steady_clock::now();
  //DensOpt.Compute_optimal_hpars(); //nécessaire avant initialise_GPs.

  auto mlsl=chrono::steady_clock::now();
  //cout << "temps : " << chrono::duration_cast<chrono::seconds>(mlsl-sbplx).count() << " s" << endl;
  cout << "Gradis" << endl;
  DensOpt.Compute_optimal_hpars(nlopt::GD_MLSL); //nécessaire avant initialise_GPs.
  auto stogo=chrono::steady_clock::now();
  cout << "temps : " << chrono::duration_cast<chrono::seconds>(stogo-mlsl).count() << " s" << endl;
