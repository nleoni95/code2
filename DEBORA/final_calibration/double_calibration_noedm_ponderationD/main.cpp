...
// sans erreur de modèle : on va mettre no prior, et des bornes pour sigma edm hyper faibles.
...
double Kernel_Z_noedm(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //squared exponential
  return 0;
}
...
double optfuncKOH_Double(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  VectorXd hpars_alpha=h.head(1);
  VectorXd hpars_diam=h.tail(1);
...
VectorXd HparsKOH_double(VectorXd const & hpars_guess,VectorXd const & lb_hpars,VectorXd const & ub_hpars,Density const & D1, Density const & D2,vector<AUGDATA> const & data_exp_1,vector<AUGDATA> const & data_exp_2){
  ...
  guess=optroutine(optfuncKOH_Double,&tp,guess,lb_hpars,ub_hpars,300); //max_time en secondes
  return guess;
}
...
int main(int argc, char **argv){
...

  {
    ...
    MainDensity_alpha.SetKernel(Kernel_Z_noedm);
    ...
    MainDensity_diam.SetKernel(Kernel_Z_noedm);
    ...
    auto in_bounds=[&MainDensity_alpha,&MainDensity_diam,lb_hpars_alpha,lb_hpars_diam,ub_hpars_alpha,ub_hpars_diam](VectorXd const & X){
      if(X.size()==5){
        return MainDensity_alpha.in_bounds_pars(X);
      }
      if(X.size()==7){
        //version spéciale no edm
        VectorXd Xa=0.5*(lb_hpars_alpha+ub_hpars_alpha);
        VectorXd Xd=0.5*(lb_hpars_diam+ub_hpars_diam);
        Xa(0)=X(5);
        Xd(0)=X(6);
        return MainDensity_alpha.in_bounds_pars(X.head(5)) && MainDensity_diam.in_bounds_hpars(Xd) && MainDensity_alpha.in_bounds_hpars(Xa);
      }
    ...
    VectorXd hpars_double_guess(2);
    hpars_double_guess << hpars_z_guess_alpha(0),hpars_z_guess_diam(0);
    VectorXd lb_hpars_double(2);
    lb_hpars_double << lb_hpars_alpha(0),lb_hpars_diam(0);
    VectorXd ub_hpars_double(2);
    ub_hpars_double << ub_hpars_alpha(0),ub_hpars_diam(0);
  ...
  /*
    ...
    cout << "hpars koh double after optimisation : " << hparskohdouble.transpose() << endl;
      ...
      VectorXd hparskoh_alpha=hparskohdouble.head(dim_hpars);
      VectorXd hparskoh_diam=hparskohdouble.tail(dim_hpars);
    ...
    
    {
      ...
      auto begin=chrono::steady_clock::now();
        ...
        auto compute_score_fb=[&MainDensity_diam,&MainDensity_alpha](pair<VectorXd,VectorXd> p, VectorXd const &X){
          ...
          VectorXd hpars_diam=X.tail(1);
          VectorXd hpars_alpha(1);
          hpars_alpha << X(5);
        ...
        VectorXd Xinit_fb(7);
        Xinit_fb << Xinit(0),Xinit(1),Xinit(2),Xinit(3),Xinit(4),hpars_z_guess_alpha(0),hpars_z_guess_diam(0);

        MatrixXd COV_init_fb=MatrixXd::Identity(7,7);
        COV_init_fb.topLeftCorner(5,5)=0.5*COV_init;
        COV_init_fb(5,5)*=pow(1e-4,2); //sedm bougé
        COV_init_fb(6,6)*=pow(4e-8,2); //sobs
        COV_init_fb*=0.5;
