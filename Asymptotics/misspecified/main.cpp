...
double GradKernel_Z_SE(VectorXd const &x, VectorXd const &y, VectorXd const &hpar,int index){
  //gradients du kernel SE.
  double d = abs(x(0) - y(0));
  if(index==0){
    return 2*hpar(0) * exp(-0.5 * pow(d / hpar(1), 2));
  }
  else if(index==1){
    return pow(hpar(0), 2)*pow(d,2)*pow(hpar(1),-3)*exp(-0.5 * pow(d / hpar(1), 2));
  }
  else{
...
int main(int argc, char **argv)
{
  ...
  double noise = sqrt(0.001); //sigma noise
  int nombre_steps_mcmc = 2e4;
  ...
  COV_init(0, 0) = pow(0.1, 2); //pour KOH separate : 1e-2 partout fonctionne bien.
  COV_init(1, 1) = pow(0.7, 2);
  COV_init(2, 2) = pow(2, 2);
  ...
  auto run_analysis = [noise, &generator, nombre_steps_mcmc, nombre_samples_collected, lb_t, ub_t, lb_hpars, ub_hpars, dim_theta, dim_hpars, lambda_model, X_init, COV_init,hpars_z_guess](string foldname, VectorXd const &Xobs, VectorXd const &Yobs)
  {
    ...
    for(int i=0;i<chainsize;i++){
      ...
      lmean+=pow(10,visited_steps[i](2));
    }
    tmean/=chainsize; smean/=chainsize; lmean/=chainsize;
    for(int i=0;i<chainsize;i++){
      tstd+=pow(visited_steps[i](0)-tmean,2);
      sstd+=pow(pow(10,visited_steps[i](1))-smean,2);
      lstd+=pow(pow(10,visited_steps[i](2))-lmean,2);
    }
    ...
    VectorXd res(6);
    res << tmean, tstd,smean,sstd,lmean,lstd;
...
/*
  ...
  */
  ...
  for(int i:indexes){
    ...
    ofile << i << " " << v(0) << " " << v(1) << " " << v(2) << " " << v(3) <<" " << v(4) << " " << v(5) << endl;
  }
  exit(0);
