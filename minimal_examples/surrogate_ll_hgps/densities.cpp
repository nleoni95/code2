...
Eigen::MatrixXd Density::Gamma(vector<VectorXd> const & locs, Eigen::VectorXd const &hpar) const {
  ...
  for(int i=0; i<nd; i++){
	...
	A(i,j) += pow(noise,2);		//bruit exp toujours à la position 1
      }
    }
  }
  return A;
...
double Density::loglikelihood_theta_fast(Eigen::VectorXd const &theta,Eigen::VectorXd const &hpars, Eigen::LDLT<Eigen::MatrixXd> const &ldlt)const{
    ...
    VectorXd obs=Vals-m_model(X,theta)-m_priormean(X,hpars);
    //cout << "x : " << X.transpose() << endl;
    //cout << "obs : " << Vals.transpose() << endl;
    //cout << "mod preds : " << m_model(X,theta).transpose() << endl;
...
double Density::optfuncKOH(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  for(int i=0;i<prob.size();i++){
    ...
    if(isinf(f)){cerr << "erreur myoptfunc_koh : infini. Valeur de la fonction : " << p+logprior << " " << p << " " << logprior << endl;}
    prob[i]=f;
  }
...
VectorXd DensityOpt::opti_1gp(int i, VectorXd & hpars_guess){
  ...
  cout  << "par before opt : " << hpars_guess.transpose() << endl;
  auto begin=chrono::steady_clock::now();
...
Eigen::VectorXd DensityOpt::intel_EvaluateHparOpt(std::vector<GP> const & gps,Eigen::VectorXd const & theta) const{
  //prédiction moyenne à partir d'un vecteur quelconque
  VectorXd Pred(m_dim_hpars);
  for(int i=0;i<m_dim_hpars;i++){
    Pred(i)=gps[i].EvalMean(theta)*m_Acoefs(i,i)+m_featureMeans(i);
  }
  return Pred;
}

vector<double> DensityOpt::intel_updated_var(vector<GP> const & gps, vector<Eigen::VectorXd> const & thetas_grid, Eigen::VectorXd const & new_theta)const{

  //calcul de la variance sur le grid, en rajoutant dans les hGPs le point theta. important. faisons déjà sans optimisation.
  //on update les hGPs du new_theta. Il n'y a pas besoin d'évaluer normalement ? faisons sale.
  //un peu sale. On évalue les GPs en theta, et on fait comme si c'était uen observation. De toute façon on ne récupère que la predictive variance, qui donc ne dépend pas de la valeur de l'observation, mais de sa location.
  VectorXd new_hpars=intel_EvaluateHparOpt(gps,new_theta);

  //on créé une observation et on l'intègre aux gps.
  //on reprend un peu du code du buildgp. Il faut normaliser.
  new_hpars-=m_featureMeans;
  for(int i=0;i<new_hpars.size();i++){new_hpars(i)/=m_Acoefs(i,i);}
  vector<GP> updated_gps=gps;

  for(int i=0;i<gps.size();i++){
    GP ugp(gps[i]);
    DATA dat; dat.SetX(new_theta); dat.SetValue(new_hpars(i));
    vector<DATA> d; d.push_back(dat);
    ugp.AddData(d);
    ugp.SetGP(ugp.GetPar());
    updated_gps.push_back(ugp);
  }

  //et maintenant prédiction des variances
  vector<double> variances;
  for(int i=0;i<thetas_grid.size();i++){
    VectorXd theta=thetas_grid[i];
    //calcul des variances individuelles.
    VectorXd IndivVariances(m_dim_hpars);
    
    for(int j=0;j<m_dim_hpars;j++){
      IndivVariances(j)=updated_gps[j].Eval(theta)(1);
    }
    //cout << "indivvariances sur theta = " << theta(0) << " "<< IndivVariances.transpose() << endl;
    //les variances individuelles seraient multipliées par pow(m_Acoefs(i,i),2). Mais on préfère les garder du même ordre de grandeur pour faire un seul estimateur.
    double var_final=IndivVariances.array().mean();
    variances.push_back(var_final);
  }
  return variances;
}

void DensityOpt::intel_Run_Burn_MCMC(int nburn, Eigen::MatrixXd & COV_init,Eigen::VectorXd &Xinit,std::default_random_engine & generator)const{
   //phase de burn et calcul de la nouvelle matrice de covariance.
  //on renvoie la covariance scalée et l'état actuel.
  //on peut calculer une seule fois la matrice Gamma puisque les hpars restent identiques.
  auto compute_score=[this](VectorXd const & hpars,VectorXd const & Xtest)-> double {
...
std::tuple<std::vector<Eigen::VectorXd>,std::vector<Eigen::VectorXd>> DensityOpt::intel_Run_MCMC(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator)const {
  //obtenir un sample de thetas à une étape n
  ...
  normal_distribution<double> distN(0,1);
  MatrixXd COV=COV_init;
  intel_Run_Burn_MCMC(nsteps*0.1,COV,Xinit,generator);
  VectorXd hpars_init=EvaluateHparOpt(Xinit);
  //COV=scale_covmatrix(COV,Xinit,compute_score,0,generator,"results/diag/scaleopt.gnu");
  MatrixXd sqrtCOV=COV.llt().matrixL();
  double finit=compute_score(hpars_init,Xinit);
  ...
  double acc_rate=(double)(naccept)/(double)(nsteps);
  cout << "MCMC phase over. " << " time : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << " s, accept rate : " << 100*acc_rate << " pct. " << endl;
  cout << "number of selected samples : " << selected_samples.size() << endl;
  cout << selected_samples[0].transpose() << endl;
  cout << selected_samples[1].transpose() << endl;
  cout << selected_samples[2].transpose() << endl;
  cout << selected_samples[3].transpose() << endl;
  cout << selected_samples[4].transpose() << endl;
  auto tp=make_tuple(allsamples,selected_samples);
  return tp;
} 

void DensityOpt::intel_autocor_diagnosis(vector<VectorXd> & allsamples, int nstepsmax,string const & filename) const {
  //calculer l'autocorrélation de la chaîne pour vérifier qu'elle s'est bien passée.
  //allsamples est modifiée.
  //diagnostic d'autocorrélation, jusqu'à une distance de nstepsmax pas dans la mcmc.
  //on oublie les 10% premières steps dans le diagnostic, pour la phase de burn.
  //on centre les données
  int dim_mcmc=allsamples[0].size();
  int sample_length=allsamples.size();
  VectorXd mean=VectorXd::Zero(dim_mcmc);
  VectorXd var=VectorXd::Zero(dim_mcmc); //variance composante par composante
  ...
  for (int i=0;i<sample_length;i++){
    allsamples[i]-=mean;
    var+=allsamples[i].cwiseProduct(allsamples[i]);
  }
  var/=sample_length; //calcul de la variance composante par composante.
  ofstream ofile(filename);
  VectorXd integ=VectorXd::Zero(dim_mcmc);
  FILE* out= fopen("results/autocor.gnu","w");
  for (int i=0;i<nstepsmax;i++){
    VectorXd cor=Lagged_mean(allsamples,i).cwiseQuotient(var);
    for(int j=0;j<cor.size();j++){
      ofile << cor(j) << " ";
    }
    integ+=cor;
...
std::vector<Eigen::VectorXd>::iterator DensityOpt::intel_choose_theta(std::vector<Eigen::VectorXd> const &grid_theta, std::vector<Eigen::VectorXd> const &theta_samples)const{
  //faisons une fonction simple où on ne choisit que dans le sample, déjà. 
  vector<VectorXd> v;
  return v.begin();
} 
