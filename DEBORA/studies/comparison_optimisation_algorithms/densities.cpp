...
double Density::loglikelihood_theta_fast(Eigen::VectorXd const &theta,Eigen::VectorXd const &hpars, Eigen::LDLT<Eigen::MatrixXd> const &ldlt)const{
    ...
    VectorXd obs=Vals-m_model(X,theta)-m_priormean(X,hpars); //problème ici. On voudrait mettre la priormean, sauf que ça dépend d'hpars qui ne sont pas connus dans cette fonction. On pourrait le corriger en créant les data dans le fonction d'avant, mais cela ne permettrait pas le découplage pour optfunc_opt. On reste sans priormean.
...
int Density::optroutine_withgrad(nlopt::vfunc optfunc,void *data_ptr, VectorXd &X, VectorXd const & lb_hpars, VectorXd const & ub_hpars, double max_time)const{
  ...
  opt.set_local_optimizer(local_opt);
  opt.set_stopval(475.44-tol);
  double msup; /* the maximum objective value, upon return */
  int fin=opt.optimize(x, msup); //messages d'arrêt 
  if (!fin==3){cout << "opti hpars message d'erreur : " << fin << endl;}
  X=VtoVXD(x);
  return opt.get_numevals();
}

int Density::optroutine_nograd(nlopt::vfunc optfunc,void *data_ptr, VectorXd &X, VectorXd const & lb_hpars, VectorXd const & ub_hpars, double max_time)const{
  //routine d'optimisation avec gradient
  double tol=max_time;
  vector<double> x=VXDtoV(X);
  vector<double> lb_hpars_opt=VXDtoV(lb_hpars);
  ...
  double xtol_fin=1e-7;
  // 1 optimiseur local et un global.
  nlopt::opt local_opt(nlopt::LN_SBPLX,x.size());//MMA en temps normal.
...
VectorXd DensityOpt::HparsOpt(VectorXd const & theta, VectorXd const & hpars_guess, double max_time) const{
  ...
  int nevals=optroutine_withgrad(optfuncOpt_withgrad,&p,guess,m_lb_hpars,m_ub_hpars,max_time);
  cout << "nevals : " << nevals << endl;
  return guess;
}

VectorXd DensityOpt::HparsOpt(VectorXd const & theta, VectorXd const & hpars_guess, double max_time, int &nevals) const{
  VectorXd guess=hpars_guess;
  //guess(2)=1e-3; //modif nico
  //calcul des données f-theta
  VectorXd obsmtheta(m_Xprofile.size());
  ...
  obsmtheta=Yexp-Fpred; //la priormean sera mise dans optfuncopt
  auto p=make_pair(&obsmtheta,this);
  nevals=optroutine_withgrad(optfuncOpt_withgrad,&p,guess,m_lb_hpars,m_ub_hpars,max_time);
  cout << "nevals : " << nevals << endl;
  return guess;
}

VectorXd DensityOpt::HparsOpt_nograd(VectorXd const & theta, VectorXd const & hpars_guess, double max_time) const{
  VectorXd guess=hpars_guess;
  //guess(2)=1e-3; //modif nico
  //calcul des données f-theta
  VectorXd obsmtheta(m_Xprofile.size());
  ...
  obsmtheta=Yexp-Fpred; //la priormean sera mise dans optfuncopt
  auto p=make_pair(&obsmtheta,this);
  int nevals=optroutine_nograd(optfuncOpt,&p,guess,m_lb_hpars,m_ub_hpars,max_time);
  return guess;
}

VectorXd DensityOpt::HparsOpt_nograd(VectorXd const & theta, VectorXd const & hpars_guess, double max_time, int &nevals) const{
  VectorXd guess=hpars_guess;
  //guess(2)=1e-3; //modif nico
  //calcul des données f-theta
  VectorXd obsmtheta(m_Xprofile.size());
  ...
  obsmtheta=Yexp-Fpred; //la priormean sera mise dans optfuncopt
  auto p=make_pair(&obsmtheta,this);
  nevals=optroutine_nograd(optfuncOpt,&p,guess,m_lb_hpars,m_ub_hpars,max_time);
  return guess;
}
...
double DensityOpt::optfuncOpt_withgrad(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  const vector<VectorXd> *xconv=d->GetXconverted();
  LDLT<MatrixXd> ldlt(d->Gamma(*xconv,hpars));
  int nd=obsmodif.size();
  MatrixXd Kinv=ldlt.solve(MatrixXd::Identity(nd,nd));
  VectorXd alpha=Kinv*obsmodif;
  double ll=-0.5*obsmodif.dot(alpha)-0.5*(ldlt.vectorD().array().log()).sum()-0.5*nd*log(2*3.1415);
  double lp=d->m_logpriorhpars(hpars);
...
Eigen::VectorXd DensityOpt::derivatives(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars) const {
  //calcul des dérivées à ma manière. je ne fais pas la tendance car c'est chiant.
  VectorXd y=m_data_exp[0].Value()-m_model(m_data_exp[0].GetX(),theta)-EvaluatePMean(m_data_exp[0].GetX(),hpars);
  MatrixXd G=Gamma(m_Xprofile_converted,hpars);
  LDLT<MatrixXd> ldlt(G);
  VectorXd alpha=ldlt.solve(y);
  MatrixXd Ginv=ldlt.solve(MatrixXd::Identity(G.rows(),G.cols()));
  MatrixXd Gamma2=G;
  for(int i=0;i<G.rows();i++){
    for(int j=0;j<G.cols();j++){
      Gamma2(i,j)*=pow(m_Xprofile_converted[i](0)-m_Xprofile_converted[j](0),2);
    }
  }
  VectorXd der(3);
  der(0)=(1/hpars(0))*(y.dot(alpha)-G.rows()+pow(hpars(1),2)*(Ginv.trace()-alpha.squaredNorm()))-2/hpars(0);//ne pas oublier le prior.
  der(1)=hpars(1)*(alpha.squaredNorm()-Ginv.trace());
  der(2)=(1/(2*pow(hpars(2),3))*(alpha.transpose()*Gamma2*alpha-(Ginv*Gamma2).trace()));
  return der;
}

Eigen::VectorXd DensityOpt::secondderivatives(Eigen::VectorXd const & theta, Eigen::VectorXd const & hpars) const {
  //calcul des dérivées à ma manière. je ne fais pas la tendance car c'est chiant.
  VectorXd y=m_data_exp[0].Value()-m_model(m_data_exp[0].GetX(),theta)-EvaluatePMean(m_data_exp[0].GetX(),hpars);
  VectorXd first_der=derivatives(theta,hpars);
  MatrixXd G=Gamma(m_Xprofile_converted,hpars);
  double n=G.cols();
  LDLT<MatrixXd> ldlt(G);
  VectorXd alpha=ldlt.solve(y);
  ...
  for(int i=0;i<G.rows();i++){
    for(int j=0;j<G.cols();j++){
      Gamma2(i,j)*=pow(m_Xprofile_converted[i](0)-m_Xprofile_converted[j](0),2);
      Gamma3(i,j)*=pow(m_Xprofile_converted[i](0)-m_Xprofile_converted[j](0),4);
    }
  }
  VectorXd der2(3);
  double l=hpars(2);
  double l3=pow(l,3);
  VectorXd alpha2=Gamma2*alpha;
  der2(0)=-(first_der(0)/pow(hpars(0),2))+(2/pow(hpars(0),2))*(pow(hpars(1),2)*y.dot(Ginv*Ginv*y)-y.dot(alpha)-pow(hpars(1),2)*(pow(hpars(1),2)*alpha.dot(Ginv*alpha)-alpha.squaredNorm())+pow(hpars(1),2)*(Ginv-pow(hpars(1),2)*Ginv*Ginv).trace())+2/pow(hpars(0),2); 
  //je recalculerai plus tard. ne pas oublier le prior ! 
  der2(1)=alpha.squaredNorm()-n*Ginv.trace()-2*pow(hpars(1),2)*(2*alpha.dot(Ginv*alpha)-(Ginv*Ginv).trace());
  der2(2)=-(3./(2*pow(l,4)))*(alpha.dot(Gamma2*alpha)-(Ginv*Gamma2).trace())+(1/(2*l3))*((1/l3)*(alpha.dot(Gamma3*alpha)-2*alpha2.transpose()*Ginv*alpha2)-(1/l3)*(Ginv*Gamma3-Ginv*Gamma2*Ginv*Gamma2).trace());
  return der2;
}

void DensityOpt::Compute_optimal_hpars(double max_time){
...
void DensityOpt::update_hGPs(vector<VectorXd> const &new_thetas,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes, double max_time){
  ...
  BuildHGPs(Kernel_GP,Bounds_hpars_GPs,Hpars_guess_GPs,nmodes);
}

void DensityOpt::update_hGPs(vector<VectorXd> const &new_thetas,vector<VectorXd> const &new_hpars,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes){
  //on fait une optimisation fine sur les new thetas, puis on les rajoute aux hGPs. On optimise les hyperparamètres avec le tout.
  cout << "updating hGPs with " << new_thetas.size() << "new points already computed." << endl;
  for(int i=0;i<new_thetas.size();i++){
    AUGDATA dat; dat.SetX(new_thetas[i]),dat.SetValue(new_hpars[i]);
    m_hpars_opti.push_back(dat);
  }
...
Eigen::VectorXd DensityOpt::compute_grad_analytic(VectorXd const & theta, VectorXd const & hpars) const{
  //calcul analytique du gradient.
  //il faut calculer le vecteur (obs - ftheta - beta)
  VectorXd grad(hpars.size());
  VectorXd obs=m_data_exp[0].Value()-m_model(m_data_exp[0].GetX(),theta)-EvaluatePMean(m_Xprofile,hpars);
  LDLT<MatrixXd> ldlt(Gamma(m_Xprofile_converted,hpars));
  int nd=m_Xprofile_converted.size();
  MatrixXd DG1=MatrixXd::Zero(nd,nd);
  MatrixXd DG2=MatrixXd::Zero(nd,nd);
  MatrixXd DG3=MatrixXd::Zero(nd,nd);
  for(int i=0; i<nd; i++){
    for(int j=i; j<nd; j++){
      DG1(i,j)=m_DKernel1(m_Xprofile_converted[i],m_Xprofile_converted[j],hpars);
      DG3(i,j)=m_DKernel3(m_Xprofile_converted[i],m_Xprofile_converted[j],hpars);
      if(i!=j){
        DG1(j,i) = DG1(i,j);
        DG3(j,i) = DG3(i,j);
      }else{
...
Eigen::VectorXd DensityOpt::compute_grad_finite_diff(VectorXd const & theta, VectorXd const & hpars, double pct_distance) const{
  auto compute_score=[&theta,this](VectorXd const & hpar){
    return loglikelihood_theta(theta,hpar)+EvaluateLogPHpars(hpar);
  };
  auto compute_derivative_1D=[&theta,&hpars,&compute_score](int direction, double distance){
    VectorXd hpars_low=hpars; hpars_low(direction)-=distance;
    VectorXd hpars_up=hpars; hpars_up(direction)+=distance;
    return (compute_score(hpars_up)-compute_score(hpars_low))/(2*distance);
  };
  //maintenant : comment choisir les distances ? on va faire à pct_distance.
  VectorXd dist=pct_distance*hpars;
  VectorXd grad(hpars.size());
  for(int i=0;i<grad.size();i++){
    grad(i)=compute_derivative_1D(i,dist(i));
  }
  return grad;  
}

Eigen::VectorXd DensityOpt::compute_hess_finite_diff(VectorXd const & theta, VectorXd const & hpars, double pct_distance) const{
  auto compute_score=[&theta,this](VectorXd const & hpar){
    return loglikelihood_theta(theta,hpar)+EvaluateLogPHpars(hpar);
  };
  auto compute_sderivative_1D=[&theta,&hpars,&compute_score](int direction, double distance){
    VectorXd hpars_low=hpars; hpars_low(direction)-=distance;
    VectorXd hpars_up=hpars; hpars_up(direction)+=distance;
    return (compute_score(hpars_up)+compute_score(hpars_low)-2*compute_score(hpars))/pow(distance,2);
  };
  //maintenant : comment choisir les distances ? on va faire à pct_distance.
  VectorXd dist=pct_distance*hpars;
  VectorXd grad(hpars.size());
  for(int i=0;i<grad.size();i++){
    grad(i)=compute_sderivative_1D(i,dist(i));
  }
  return grad;  
}
...
double Densities::Run_Burn_phase_MCMC_fixed_hpars(int nburn, MatrixXd & COV_init,vector<VectorXd> const & hpars_v, std::vector<Eigen::LDLT<Eigen::MatrixXd>> const & ldlt_v, VectorXd & Xcurrento, default_random_engine & generator) {
  ...
  auto compute_score=[this,hpars_v,ldlt_v](VectorXd const & theta)-> double {
    double s=0;
    for(int i=0;i<m_dim;i++){
      s+=this->loglikelihood_theta_fast(theta,hpars_v,ldlt_v);
    }
    s+=m_logpriorpars(theta);
    return s;
  };
...
void DensitiesOpt::compute_optimal_hpars(double max_time){
  for(int i=0;i<m_dim;i++){
    m_DensityOpt_vec[i].Compute_optimal_hpars(max_time);
  }
}
...
void DensitiesOpt::BuildHGPs(double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &),Eigen::MatrixXd const & Bounds_hpars_GPs,Eigen::VectorXd const & Hpars_guess_GPs,int nmodes){
  //on suppose que les kernels, bounds hpars, hpars_guess et nmodes sont pareils pour toutes les densités.
  for(int i=0;i<m_dim;i++){
    m_DensityOpt_vec[i].BuildHGPs(Kernel_GP,Bounds_hpars_GPs,Hpars_guess_GPs,nmodes);
  }
}
