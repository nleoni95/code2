...
int Density::optroutine(nlopt::vfunc optfunc,void *data_ptr, VectorXd &X, VectorXd const & lb_hpars, VectorXd const & ub_hpars, int max_time)const{
...
int Density::optroutine_withgrad(nlopt::vfunc optfunc,void *data_ptr, VectorXd &X, VectorXd const & lb_hpars, VectorXd const & ub_hpars, int max_time)const{
...
VectorXd Density::HparsKOH(VectorXd const & hpars_guess, int max_time) const {
...
VectorXd Density::HparsLOOCV(VectorXd const & hpars_guess, int max_time) const{
  //calcule les hyperparamètres optimaux selon KOH.
  VectorXd guess=hpars_guess;
  //On prépare les évaluations de surrogate pour la routine opt, pour accélérer la fonction.
  MatrixXd Residustheta(m_data_exp[0].Value().size(),m_Grid.m_grid.size());
...
VectorXd DensityOpt::HparsOpt(VectorXd const & theta, VectorXd const & hpars_guess, int max_time) const{
  VectorXd guess=hpars_guess;
...
VectorXd DensityOpt::HparsOpt_quick(VectorXd const & theta, VectorXd const & hpars_guess, int max_time) const{
  VectorXd guess=hpars_guess;
  //guess(2)=1e-3; //modif nico
  //calcul des données f-theta
  VectorXd obsmtheta(m_Xprofile.size());
...
void DensityOpt::Compute_optimal_hpars(int max_time){
...
void DensityOpt::Test_hGPs(int npoints, int max_time) {
...
void DensityOpt::update_hGPs(vector<VectorXd> const &new_thetas,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes, int max_time){
...
double DensityOpt::Run_Burn_phase_MCMC_opti_expensive(int nburn, MatrixXd & COV_init, VectorXd & Xcurrento, default_random_engine & generator, int max_time) {
  //phase de burn et calcul de la nouvelle matrice de covariance.
  //on renvoie la covariance scalée et l'état actuel.
  //on peut calculer une seule fois la matrice Gamma puisque les hpars restent identiques.
...
void DensityOpt::Run_MCMC_opti_expensive(int nsteps,int nsamples, VectorXd & Xinit, MatrixXd const & COV_init,default_random_engine & generator, int max_time){
...
void DensityOpt::Run_MCMC_opti_hGPs_adaptative(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &),Eigen::MatrixXd const & Bounds_hpars_GPs,Eigen::VectorXd const & Hpars_guess_GPs,int nmodes, int max_time){
  //On commence avec une chaîne 200 points puis on rajoute 200 pts.
  //faisons autant de steps dans l'une que dans l'autre.
  cout << "running mcmc hopti hgps adaptative with " << nsteps << "steps..." << endl;
  m_samples.clear(); m_hparsofsamples.clear(); m_allmcmcsamples.clear();
  vector<VectorXd> new_thetas;
  auto compute_score=[this](VectorXd const & hpars,VectorXd const & Xtest)-> double {
    return this->loglikelihood_theta(Xtest,hpars)+this->m_logpriorpars(Xtest);
  };
  uniform_real_distribution<double> distU(0,1);
  ...
  for(int i=0;i<nsteps;i++){
    ...
    if(i%(nsteps/200)==0 && i>0){
      new_thetas.push_back(Xcurrent);
    }
  }
  auto end=chrono::steady_clock::now();
  double acc_rate=(double)(naccept)/(double)(nsteps);
  cout << "first MCMC over. " << " time : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << " s, accept rate : " << 100*acc_rate << " pct. " << endl;
  cout << "number of new thetas : " << new_thetas.size() << endl;
  cout << new_thetas[0].transpose() << endl;
  cout << new_thetas[1].transpose() << endl;
  cout << new_thetas[2].transpose() << endl;
  cout << new_thetas[3].transpose() << endl;
  cout << new_thetas[4].transpose() << endl;
  update_hGPs(new_thetas,Kernel_GP,Bounds_hpars_GPs,Hpars_guess_GPs,nmodes,max_time);
  Run_MCMC_opti_hGPs(nsteps,nsamples,Xinit,COV_init,generator);
}
...
vector<VectorXd> Densities::HparsKOH_separate(vector<VectorXd> const & hpars_guess_vec, int max_time) {
  auto begin=chrono::steady_clock::now();
...
vector<VectorXd> Densities::HparsKOH_pooled(vector<VectorXd> const & hpars_guess_vec, int max_time) {
  auto begin=chrono::steady_clock::now();
...
int Densities::optroutine(nlopt::vfunc optfunc,void *data_ptr, vector<double> &x, vector<double> const & lb_hpars, vector<double> const & ub_hpars, int max_time){
    //routine d'optimisation sans gradient
...
void DensitiesOpt::compute_optimal_hpars(int max_time){
  for(int i=0;i<m_dim;i++){
    m_DensityOpt_vec[i].Compute_optimal_hpars(max_time);
  }
}

std::vector<Eigen::VectorXd> DensitiesOpt::HparsOpt(Eigen::VectorXd const & theta,std::vector<Eigen::VectorXd> const & hpars_guess_vec, int max_time) const{
...
void DensitiesOpt::Test_hGPs(int npoints, int max_time){
  for(int i=0;i<m_dim;i++){
