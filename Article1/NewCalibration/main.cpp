...
vector<DATA> GetObs(PyObject *pFunc_exp,int cas){
  ...
  //vector<int> added_obs={3,5,6,15,20,21}; //on rajoute 1 observation en 0 tirée du modèle, pour forcer les prédictions à avoir un cpt correct en 0.
  vector<int> added_obs={0}; //on rajoute 1 observation en 0 tirée du modèle, pour forcer les prédictions à avoir un cpt correct en 0.
  if (std::find(added_obs.begin(), added_obs.end(), cas) != added_obs.end()) {
    //le cas fait partie de ceux dont on veut rajouter une obs.
    VectorXd par=0.5*VectorXd::Ones(3); VectorXd x=VectorXd::Zero(1); x <<0.2;
    DATA dat; dat.SetX(x); dat.SetValue(ravik_model_physique(0.2,GPtoR(par),cas));
    //rajouter la data dans le vecteur d'observations
    vector<DATA> data2(PyList_Size(pValue)+1);
    data2[0]=dat;
    for (int i=0;i<PyList_Size(pValue);i++){
      data2[i+1]=data[i];
    }
    cout << "case: " << cas <<"number of obs total : " << data2.size() << endl;
    return data2;
  }
...
double optfuncKOH_pooled(const std::vector<double> &x, std::vector<double> &grad, void *data){
  ...
  for(int i=0;i<d->size();i++){
    ...
    MatrixXd M=d->at(i).Gamma(xlocs,hpars_v[i])+pow(hpars_v[i](0),2)*d->at(i).Get_IncX();
    LDLT<MatrixXd> L(M);
    ldlt_v.push_back(L);
  }
...
double optfuncKOH_pooled_callback(const std::vector<double> &x, std::vector<double> &grad, void *data){
  /* fonction à optimiser pour trouver les hpars koh.*/ //le string n'est pas encore écrit.
  auto ptp=(tuple<const vector<MatrixXd> *,vector<Density>*,string*>*) data; //cast
  auto tp=*ptp;
  const vector<MatrixXd> *Residus_v=get<0>(tp);
  vector<Density> *d=get<1>(tp);
  string *filename=get<2>(tp);
  //transformer x en vector de vectorXd pour avoir tous les hpars
  vector<VectorXd> hpars_v;
  int c=0;
  for(int i=0;i<d->size();i++){
  ...
  res=log(res);
  for(int i=0;i<d->size();i++){
    res+=d->at(i).EvaluateLogPHpars(hpars_v[i]);
  }

  //on append la chaîne de caractères au fichier de sauvegarde.
  ofstream ofile;
...
vector<VectorXd> HparsKOH_pooled_withcallback(vector<Density>& vDens,VectorXd const & hpars_guess, double max_time,string filename) {
  //optimisation KOH poolée entre toutes les densités. Il faut faire quoi alors ? On est obligé de calculer toutes les valeurs y-ftheta avant de passer à l'optfct. Sinon ça prend trop de temps.
  //avec callback : chaque itération, on écrit le point visité. Ainsi, on peut surveiller quel est le current max.
  ...
  auto tp=make_tuple(&residus_v,&vDens,&filename);
  //création des bornes des hpars et du guess. tout est fait en vector pour pouvoir gérer les tailles sans s'embêter.
  vector<double> lb_hpars,ub_hpars,guess;
  for(int i=0;i<dim;i++){
    auto p=vDens[i].GetBoundsHpars();
  ...
  int fin=optroutine(optfuncKOH_pooled_callback,&tp,guess,lb_hpars,ub_hpars,max_time);
  cout << "fin de l'opt koh pooled : message " << fin << endl;
  //il faut repasser guess en vector<vectorXd>.
  vector<VectorXd> ret;//=hpars_guess;
  int c=0;
...
int main(int argc, char **argv){
  ...
  VectorXd Xmin=0.5*VectorXd::Ones(3); Xmin << 0,0.5*18.9E-6,0.5*0.75;
  VectorXd Xmax=0.5*VectorXd::Ones(3); Xmax << 90,2*18.9E-6,2*0.75;
  ...
  VectorXd lb_t_true(dim_theta); lb_t_true << 0,0.2*18.9E-6,0.2*0.75;
  VectorXd ub_t_true(dim_theta); ub_t_true << 90,2*18.9E-6,2*0.75;
  ...
  //VectorXd lb_hpars(2); lb_hpars(0)=0;lb_hpars(1)=1;
  //VectorXd ub_hpars(2); ub_hpars(0)=1e5;ub_hpars(1)=50; //bornes sur l'optimisation des hpars. pour kernel matern.

  //linéaire
  //VectorXd lb_hpars(3); lb_hpars(0)=0.1;lb_hpars(1)=0;lb_hpars(2)=0;
  //VectorXd ub_hpars(3); ub_hpars(0)=0.8;ub_hpars(1)=1e6;ub_hpars(2)=1e4;

  VectorXd lb_hpars(3); lb_hpars(0)=0.1;lb_hpars(1)=0;lb_hpars(2)=1;//lb_hpars(2)=0;
  VectorXd ub_hpars(3); ub_hpars(0)=0.8;ub_hpars(1)=1e6;ub_hpars(2)=30;//ub_hpars(2)=1e6;
...
//vector<int> cases={3,4,5,6,8,15,16,20,21}; // total
...
vector<int> cases={6};
  ...
  for(int i=0;i<cases.size();i++){
      ...
      auto lambda_model=[&pFunc_model,&pParamsRavik,&cases,i](VectorXd const &X, VectorXd const & theta) mutable{
    ...
    MainDensity.SetKernel(Kernel_Z_SE_lin);
...
//début des calibrations.
 ...
 //MCMC koh separate sur les densités séparées
    ...
    auto hparskoh_separate=HparsKOH_separate(vDens,hpars_z_guess,60*cases.size());
...
//calibration koh separate
  {
    ...
    auto res=Run_MCMC(nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc,COV_init,compute_score_kohs,get_hpars_kohs,in_bounds,generator);
      ...
      //écriture des samples. Il faut une fonction dédiée.
        ...
        string fnamesF="results/separatedensities/kohs/sampsF"+to_string(cases[i])+".gnu";
      string fnamesZ="results/separatedensities/kohs/sampsZ"+to_string(cases[i])+".gnu";
        ...
        vDens[i].WriteSamplesFandZ(XPREDS,fnamesF,fnamesZ);
        //prédictions
        //écriture des observations ave erreur expérimentale KOH
      string filename="results/obs"+to_string(cases[i])+".gnu";
      vDens[i].WriteObsWithUncertainty(filename,hparskoh_separate[i](0));
  }
  }

///////////////////:
//calibration avec hpars1
/*
VectorXd hpars1(3); hpars1 << 0.48,0,15;
  {
    for(int i=0;i<vsize;i++){
    auto in_bounds=[&vDens](VectorXd const & X){
      return vDens[0].in_bounds_pars(X);
    ...
    auto get_hpars_kohs=[&hpars1,&i](VectorXd const & X){
      vector<VectorXd> h(1);
      h[0]=hpars1;
      return h;
    };
    ...
    cout << "début mcmc densité hpars1 expérience" << cases[i] << endl;
    auto res=Run_MCMC(nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc,COV_init,compute_score_kohs,get_hpars_kohs,in_bounds,generator);
      auto samples_opti=get<0>(res);
      vector<VectorXd> allsamples_opti=get<2>(res);
      vector<vector<VectorXd>> vhpars_opti_noninverted=get<1>(res); //là le premier vecteur c'est les steps, et le second vecteur les densités. Il faut inverser ça...
      ...
      vDens[i].SetNewAllSamples(allsamples_opti);
      //diagnostic
      vDens[i].Autocor_diagnosis(nautocor,"results/separatedensities/hpars1/autocor"+to_string(cases[i])+".gnu");
      //écriture des samples. Il faut une fonction dédiée.
        string fname="results/separatedensities/hpars1/samp"+to_string(cases[i])+".gnu";
        string fnameall="results/separatedensities/hpars1/allsamp"+to_string(cases[i])+".gnu";
        string fnamepred="results/separatedensities/hpars1/preds"+to_string(cases[i])+".gnu";
        string fnamepredF="results/separatedensities/hpars1/predsF"+to_string(cases[i])+".gnu";
        string fnamesF="results/separatedensities/hpars1/sampsF"+to_string(cases[i])+".gnu";
      string fnamesZ="results/separatedensities/hpars1/sampsZ"+to_string(cases[i])+".gnu";
        vDens[i].WriteSamples(fname);
        vDens[i].WriteMCMCSamples(fnameall);
...
*/
///////////////////:
//calibration avec hpars2
/*
VectorXd hpars2(3); hpars2 << 0.15,48500,8.26;
  {
    for(int i=0;i<vsize;i++){
    auto in_bounds=[&vDens](VectorXd const & X){
      return vDens[0].in_bounds_pars(X);
    ...
    auto get_hpars_kohs=[&hpars2,&i](VectorXd const & X){
      vector<VectorXd> h(1);
      h[0]=hpars2;
      return h;
    };
    ...
    cout << "début mcmc densité hpars2 expérience" << cases[i] << endl;
    auto res=Run_MCMC(nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc,COV_init,compute_score_kohs,get_hpars_kohs,in_bounds,generator);
      auto samples_opti=get<0>(res);
      vector<VectorXd> allsamples_opti=get<2>(res);
      vector<vector<VectorXd>> vhpars_opti_noninverted=get<1>(res); //là le premier vecteur c'est les steps, et le second vecteur les densités. Il faut inverser ça...
      ...
      vDens[i].SetNewAllSamples(allsamples_opti);
      //diagnostic
      vDens[i].Autocor_diagnosis(nautocor,"results/separatedensities/hpars2/autocor"+to_string(cases[i])+".gnu");
      //écriture des samples. Il faut une fonction dédiée.
        string fname="results/separatedensities/hpars2/samp"+to_string(cases[i])+".gnu";
        string fnameall="results/separatedensities/hpars2/allsamp"+to_string(cases[i])+".gnu";
        string fnamepred="results/separatedensities/hpars2/preds"+to_string(cases[i])+".gnu";
        string fnamepredF="results/separatedensities/hpars2/predsF"+to_string(cases[i])+".gnu";
        string fnamesF="results/separatedensities/hpars2/sampsF"+to_string(cases[i])+".gnu";
      string fnamesZ="results/separatedensities/hpars2/sampsZ"+to_string(cases[i])+".gnu";
        vDens[i].WriteSamples(fname);
        vDens[i].WriteMCMCSamples(fnameall);
...
//Phase Opti
  ...
  DoE doe_new(lb_t,ub_t,100,10);//doe halton. 300 points en dimension 3 (3 paramètres incertains).
  for(int i=0;i<vsize;i++){
    DensityOpt Dopt(vDens[i]);
    Dopt.SetNewDoE(doe_new);
    string fname="results/hparsopt"+to_string(cases[i])+".gnu";
    Dopt.Compute_optimal_hpars(1e-3,fname);
    ...
    for(int i=0;i<vsize;i++){
      ...
      auto get_hpars_opti=[&vDopt,&i,&lb_hpars,&ub_hpars](VectorXd const & X){
        ...
        for(int j=0;j<init.size();j++){
            init(j)=max(init(j),lb_hpars(j));
            init(j)=min(init(j),ub_hpars(j));
        }
      ...
      */
      ...
      auto res=Run_MCMC(2*nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc,COV_init,compute_score_opti,get_hpars_opti,in_bounds,generator);
      auto samples_opti=get<0>(res);
      vector<VectorXd> allsamples_opti=get<2>(res);
      ...
      string fnamesF="results/separatedensities/opt/sampsF"+to_string(cases[i])+".gnu";
      string fnamesZ="results/separatedensities/opt/sampsZ"+to_string(cases[i])+".gnu";
  ...
  //Bayes : il faut faire l'exporation sur log sigma. sinon trop dur à explorer.
  {
    ...
    X_init_mcmc_fb(3)=0.5; X_init_mcmc_fb(4)=1e5;X_init_mcmc_fb(5)=25;//X_init_mcmc_fb(5)=1e4; //échelle log pour sigma
    COV_init_fb.topLeftCorner(3,3)=COV_init;
    int m=lb_hpars.size();
    COV_init_fb(3,3)=pow(0.01,2); 
    COV_init_fb(4,4)=pow(1e4,2);
    COV_init_fb(5,5)=pow(0.5,2);
...
for(int i=0;i<vsize;i++){
    ...
    auto in_bounds_fb=[&vDens,&vsize,dim_hpars](VectorXd const & X){
      VectorXd hp=X.tail(dim_hpars);;
      VectorXd theta=X.head(3);
      return vDens[0].in_bounds_pars(X) && vDens[0].in_bounds_hpars(hp) ;
    };
    ...
    auto compute_score_fb=[&vsize,&lb_hpars,&vDens,&i,dim_hpars](vector<VectorXd> const & p, VectorXd const &X){
    ...
    auto res=Run_MCMC(20*nombre_steps_mcmc,nombre_samples_collected,X_init_mcmc_fb,COV_init_fb,compute_score_fb,get_hpars_fb,in_bounds_fb,generator);
      ...
      string fnamesF="results/separatedensities/fb/sampsF"+to_string(cases[i])+".gnu";
      string fnamesZ="results/separatedensities/fb/sampsZ"+to_string(cases[i])+".gnu";
      vDens[i].WriteSamples(fname);
      vDens[i].WriteMCMCSamples(fnameall); 
...
exit(0);
  ...
  // calcul hpars ko pooled avec callback
  cout << "début koh pooled" << endl;
  string fnamekohpooled="results/fulldensities/kohpooled/hparsmonitoring.gnu";
  auto hparskoh_pooled=HparsKOH_pooled_withcallback(vDens,hpars_z_guess,3600,fnamekohpooled);
  cout << "hparskoh pooled:" << hparskoh_pooled[0].transpose() << endl;


  //ensuite : tri du fichier hparskohpooled pour afficher uniquement le maximum local.
  {
    string fnamekohpooled_clean="results/fulldensities/kohpooled/hparsmonitoringcleaned.gnu";
    ifstream ifile(fnamekohpooled);
    ofstream ofile(fnamekohpooled_clean);
    double res_max=-999;
    vector<double> xmax;
    if(ifile){
      string line;
      while(getline(ifile,line)){
        istringstream iss(line);
        vector<string> words((istream_iterator<string>(iss)),istream_iterator<string>());
        int res=stod(words[0]);
        if(res>res_max){
          res_max=res; xmax.clear(); for(int i=0;i<words.size();i++){xmax.push_back(stod(words[i]));}
        }
...
exit(0);
  ...
  //MCMC opt sur l'ensemble des densités. 
...

  {
    ...
    auto get_hpars_opti=[&vDopt,&vsize,&lb_hpars,&ub_hpars](VectorXd const & X){
      ...
      for(int i=0;i<vsize;i++){
        ...
        p[i]=vDopt[i].HparsOpt(X,init,5e-4);
      }
      return p;
    };
...
/*
    {
      ...
      for(int luck=0;luck<nrepet;luck++ ){
        ...
        auto res=Run_MCMC(10*nombre_steps_mcmc,nombre_samples_collected,starting_points[luck],COV_fromFMP,compute_score_fb,get_hpars_fb,in_bounds_fb,generator);
          ...
          auto begin=chrono::steady_clock::now();
          vDens[0].Autocor_diagnosis(20000,"results/fulldensities/multifb/autocor"+to_string(luck)+".gnu");
          auto end=chrono::steady_clock::now();
          cout << "temps pour calcul autocor bayes:. " << " time : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << " s" <<  endl;
          string fnameall="results/fulldensities/multifb/allsamp"+to_string(luck)+".gnu";
...
//profitons-en pour le calcul du critère de Rubin
{
  ...
  for(int i=0;i<nrepet;i++){
    string fname="results/fulldensities/multifb/allsamp"+to_string(i)+".gnu";
...
//étape après les multiples MCMC Bayes. constitution du sample multifb, puis prédictions sur chaque densité. on a 10 chaînes de 5e6 steps, on prend 100 points par chaîne. Donc 1 tous les 50000. 
//extraire intelligemment pour rester dans la mémoire d'un terminal.
//je modifie pour ne prédire que sur les expériences qui nous intéressent.
{
  int nrepet=10;
  ...
  for(int i=0;i<nrepet;i++){
    ...
    for (int j=0;j<100;j++){
      ...
      VectorXd hfile=s[index].tail(vsize*2);
      sample_h.push_back(hfile);
    }
  }
  ...
  for(int i=0;i<vsize;i++){
      ...
      string fname="results/fulldensities/multifb/samp"+to_string(cases[i])+".gnu";
      string fnamepredF="results/fulldensities/multifb/predsF"+to_string(cases[i])+".gnu";
      string fnamepred="results/fulldensities/multifb/preds"+to_string(cases[i])+".gnu";
      vDens[i].WriteSamples(fname);
      vDens[i].WritePredictionsF(XPREDS,fnamepredF);
      vDens[i].WritePredictions(XPREDS,fnamepred); 
  }
...
//MCMC phase Bayes. sur la densité complète. oskour. 

  {
    ...
    auto in_bounds_fb=[&vDens,&vsize,&lb_hpars,&ub_hpars](VectorXd const & X){
      VectorXd theta=X.head(3);
      vector<VectorXd> hp;
      for(int i=0;i<vsize;i++){
        VectorXd h(lb_hpars.size());
      ...
      in= in && vDens[0].in_bounds_pars(X);
      return in;
    };
    ...
    generator.seed(42);
    //une unique Bayes. Trop long à faire car mélange mal.
