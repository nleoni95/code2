...
int Density::optroutine_withgrad(nlopt::vfunc optfunc,void *data_ptr, VectorXd &X, VectorXd const & lb_hpars, VectorXd const & ub_hpars){
  ...
  opt.set_lower_bounds(lb_hpars_opt);
  opt.set_upper_bounds(ub_hpars_opt);
  opt.set_maxtime(2); //2 secondes au max.
...
void Density::WritePredictionsF(VectorXd const & X, string const & filename) const{
  ...
  int ind_map=MAP_ll();
  VectorXd predf_map=m_model(X,m_samples[ind_map]);
...
void DensityOpt::update_hGPs(vector<VectorXd> const &new_thetas,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &), MatrixXd const & Bounds_hpars_GPs,VectorXd const & Hpars_guess_GPs,int nmodes){
  ...
  for(int i=0;i<new_thetas.size();i++){
    new_hpars[i]=HparsOpt(new_thetas[i],hpars_guess);
  }
  for(int i=0;i<new_thetas.size();i++){
...
void DensityOpt::Run_MCMC_opti_hGPs_adaptative(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &),Eigen::MatrixXd const & Bounds_hpars_GPs,Eigen::VectorXd const & Hpars_guess_GPs,int nmodes){
  ...
  update_hGPs(new_thetas,Kernel_GP,Bounds_hpars_GPs,Hpars_guess_GPs,nmodes);
...
vector<VectorXd> Densities::HparsKOH_separate(vector<VectorXd> const & hpars_guess_vec) {
  auto begin=chrono::steady_clock::now();
  ...
  for(int i=0;i<m_dim;i++){
    ...
    hpars[i]=m_Densities_vec[i].HparsKOH(hpars_guess);
  }
  auto end=chrono::steady_clock::now();
  cout << "time : " << chrono::duration_cast<chrono::seconds>(end-begin).count() << " s." << endl;
  return hpars;
...
vector<VectorXd> Densities::HparsKOH_pooled(vector<VectorXd> const & hpars_guess_vec) {
  auto begin=chrono::steady_clock::now();
  ...
  int fin=optroutine(optfuncKOH_pooled,&tp,guess,lb_hpars,ub_hpars);
...
int Densities::optroutine(nlopt::vfunc optfunc,void *data_ptr, vector<double> &x, vector<double> const & lb_hpars, vector<double> const & ub_hpars){
  ...
  //opt.set_maxtime(20); //20 secondes au max.
  opt.set_local_optimizer(local_opt);
  double msup; /* the maximum objective value, upon return */
  int fin=opt.optimize(x, msup); //messages d'arrêt 
  if (!fin==3){cout << "opti hpars message d'erreur : " << fin << endl;}
...
void Densities::Run_MCMC_fixed_hpars(int nsteps,int nsamples, VectorXd & Xinit, MatrixXd const & COV_init,vector<VectorXd> const & hpars_v,default_random_engine & generator){
  ...
  cout << "running mcmc fixedhpars with " << nsteps << " steps." <<endl;
  //vidons les samples juste au cas où.
  m_samples.clear(); m_hparsofsamples_v.clear(); m_allmcmcsamples.clear();
  uniform_real_distribution<double> distU(0,1);
  normal_distribution<double> distN(0,1);
  ...
  for(int i=0;i<nsteps;i++){
    ...
    if(i%(nsteps/nsamples)==0 && i>0){
      m_samples.push_back(Xcurrent);
      m_hparsofsamples_v.push_back(hpars_v);
    }
    m_allmcmcsamples.push_back(Xcurrent);
  }
  auto end=chrono::steady_clock::now();
...
void Densities::WriteSamples(string const & filename)const{
  ...
  for(int i=0;i<m_samples.size();i++){
    ...
    for(int j=0;j<m_dim;j++){
      VectorXd h=m_hparsofsamples_v[i][j];
