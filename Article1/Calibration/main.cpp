...
double Kernel_Z_lin(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //linear
  return pow(hpar(1),2)+pow(hpar(0),2)*x(0)*y(0); //3/2
}

double Kernel_Z_quad(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //linear
  return pow(hpar(0),2)*x(0)*y(0)+pow(hpar(1),2)*pow(x(0)*y(0),2); //3/2
}
...
double Kernel_Z_SE_lin(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  ...
  return pow(hpar(0),2)*exp(-0.5*pow(d/hpar(1),2))*x(0)*y(0); //3/2
}
...
double logprior_hpars_lin(VectorXd const &hpars){
  return 0;
}
...
int main(int argc, char **argv){
  ...
  double inputerr=0.5; //sigma=0.1K d'incertitude en input.
  ...
  VectorXd lb_hpars(2); lb_hpars(0)=0;lb_hpars(1)=0;
  VectorXd ub_hpars(2); ub_hpars(0)=1e3;ub_hpars(1)=1e3;
...
//vector<int> cases={6}; // total
  ...
  for(int i=0;i<cases.size();i++){
    ...
    MainDensity.SetKernel(Kernel_Z_quad);
    ...
    MainDensity.SetLogPriorHpars(logprior_hpars_lin);
...
//Phase Opti
   ...
   //MCMC opt sur les densités séparées.calibration opti densités séparées
  {
    ...
    for(int i=0;i<vsize;i++){
      ...
      auto get_hpars_opti=[&vDopt,&i,&lb_hpars,&ub_hpars](VectorXd const & X){
        ...
        p[0]=vDopt[i].HparsOpt(X,init,1e-5);
        return p;
      };
  ...
  //Bayes : il faut faire l'exporation sur log sigma. sinon trop dur à explorer.
  {
    ...
    X_init_mcmc_fb(3)=1e2; X_init_mcmc_fb(4)=1e2; //échelle log pour sigma
    COV_init_fb.topLeftCorner(3,3)=COV_init;
    int m=lb_hpars.size();
    COV_init_fb(3,3)=pow(1e1,2); 
    COV_init_fb(4,4)=pow(1e1,2);
...
for(int i=0;i<vsize;i++){
    ...
    auto compute_score_fb=[&vsize,&lb_hpars,&vDens,&i](vector<VectorXd> const & p, VectorXd const &X){
