...
// on calibre avec un terme constant.
...
double Kernel_GP_Matern32(VectorXd const &x, VectorXd const &y, VectorXd const &hpar){
  //noyau pour le GP. 0:intensity, 2:noise, 1:lcor phi, 3:lcor BK, 4:lcor COAL, 5:lcor NUCL, 6: lcor MT
  double cor=pow(hpar(0),2);
  cor*=(1+abs(x(0)-y(0))/hpar(1)); //phi
  cor*=(1+abs(x(1)-y(1))/hpar(3)); //BK
  cor*=(1+abs(x(2)-y(2))/hpar(4)); //COAL
  cor*=(1+abs(x(3)-y(3))/hpar(5)); //NUCL
  cor*=(1+abs(x(4)-y(4))/hpar(6)); //MT
  double e=-abs(x(0)-y(0))/hpar(1);
  e-=abs(x(1)-y(1))/hpar(3);
  e-=abs(x(2)-y(2))/hpar(4);
  e-=abs(x(3)-y(3))/hpar(5);
  e-=abs(x(4)-y(4))/hpar(6);
  return cor*exp(e);
}
...
int main(int argc, char **argv){
  ...
  COV_init_bayes(8,8)*=pow(1e-5,2); //lcor avant 1e-3
  ...
  DensOpt.BuildHGPs(Kernel_GP_Matern32,Bounds_hpars_gp,hpars_gp_guess,4);
  ...
  //full bayes
  VectorXd Xinit_bayes(9);
  Xinit_bayes << 0.5,0.5,0.5,0.5,0.5,3e-4,7e-7,5e-3,0;
  MainDensity.Run_FullMCMC(8*nombre_steps_mcmc,nombre_samples_collected,Xinit_bayes,COV_init_bayes,generator);
