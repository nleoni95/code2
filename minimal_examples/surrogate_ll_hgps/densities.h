...
class Density
{
    ...
    std::vector<Eigen::VectorXd> GetAllSamples()const{return m_allmcmcsamples;}
...
class DensityOpt : public Density{
    ...
    std::vector<GP> GethGPs()const{return m_vgp_hpars_opti;}
    void SethGPs(std::vector<GP> const & vgp){m_vgp_hpars_opti=vgp;}
    
    //tout ce qui touche à l'adaptative sampling.
    std::vector<double> intel_updated_var(std::vector<GP> const & gps,std::vector<Eigen::VectorXd> const & thetas_grid, Eigen::VectorXd const & new_theta) const; //renvoie un vecteur de sigma_(n,new_theta)(theta)^2.

    void intel_Run_Burn_MCMC(int nburn, Eigen::MatrixXd & COV_init,Eigen::VectorXd &Xcurrent,std::default_random_engine & generator)const;

    std::tuple<std::vector<Eigen::VectorXd>,std::vector<Eigen::VectorXd>> intel_Run_MCMC(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator) const ; //obtenir un sample de thetas à une étape n

    void intel_autocor_diagnosis(std::vector<Eigen::VectorXd> & allsamples,int nstepsmax,std::string const & filename) const; //calculer l'autocorrélation de la chaîne pour vérifier qu'elle s'est bien passée.
    std::vector<Eigen::VectorXd>::iterator intel_choose_theta(std::vector<Eigen::VectorXd> const &grid_theta, std::vector<Eigen::VectorXd> const &theta_samples)const; //faisons une fonction simple où on ne choisit que dans le sample, déjà. 
    Eigen::VectorXd intel_EvaluateHparOpt(std::vector<GP> const & gps,Eigen::VectorXd const & theta) const;
...
class Densities{
    ...
    void Run_MCMC_adapt(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::vector<Eigen::VectorXd> const & hpars_v,double lambda_init,double gamma,std::default_random_engine & generator);
...
class DensitiesOpt : public Densities{
    ...
    Eigen::MatrixXd Burn_phase_test(int nburn, Eigen::MatrixXd const & COV_init,Eigen::VectorXd &Xcurrent,std::default_random_engine & generator, std::string const & filename);
    ...
    void Run_MCMC_opti_noburn(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,std::default_random_engine & generator);

    void Run_MCMC_opti_adapt(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,double lambda_init,double gamma,std::default_random_engine & generator);
    
    void Run_MCMC_opti_adapt2(int nsteps,int nsamples,Eigen::VectorXd & Xinit, Eigen::MatrixXd const & COV_init,Eigen::VectorXd lambda_init,double gamma,std::default_random_engine & generator);
    
    //pour les hgps.
    std::vector<std::vector<Eigen::VectorXd>> compute_optimal_hpars(double max_time) const;

    void write_optimal_hpars(std::vector<std::vector<Eigen::VectorXd>> const & hpars, std::vector<Eigen::VectorXd> const & grid_theta,std::string filename) const;
    
    void read_optimal_hpars(std::vector<std::vector<Eigen::VectorXd>> & hpars, std::vector<Eigen::VectorXd> & grid_theta,std::string filename,int ncases, int dimhgp) const;

    void update_hGPs_with_hpars(std::vector<Eigen::VectorXd> const & grid_theta,std::vector<std::vector<Eigen::VectorXd>>  const & hparsv,double (*Kernel_GP)(Eigen::VectorXd const &, Eigen::VectorXd const &, Eigen::VectorXd const &),Eigen::MatrixXd const & Bounds_hpars_GPs,Eigen::VectorXd const & Hpars_guess_GPs);
    ...
    std::vector<Eigen::VectorXd> m_grid;//on suppose même grid pour tout le monde.
};
