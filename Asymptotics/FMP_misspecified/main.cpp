...
int main(int argc, char **argv)
{
  ...
  double noise = 0.05; //sigma noise
  int nombre_steps_mcmc = 4000;
  int nombre_samples_collected = 20; //on garde la moitié des samples de la mcmc
  int dim_theta = 1;
  int dim_hpars = 2;

  // Bornes des paramètres et hyperparamètres
  VectorXd lb_t(dim_theta);
  lb_t(0) = -1.5; //0.2 avant
  ...
  Bounds_hpars_gp(1, 0) = 5; //variance
  Bounds_hpars_gp(0, 1) = 1e-5;
  Bounds_hpars_gp(1, 1) = 10; //lcor en theta
  ...
  VectorXd X_init(1);
  X_init(0) = -1;
  MatrixXd COV_init = MatrixXd::Identity(1, 1);
  COV_init(0, 0) = pow(0.08, 2); //pour KOH separate : 1e-2 partout fonctionne bien.
  ...
  int seed = 996696; //seed pour les observations
  ...
  auto run_analysis = [noise, &generator, nombre_steps_mcmc, nombre_samples_collected,Bounds_hpars_gp,hpars_gp_guess, lb_t, ub_t, lb_hpars, ub_hpars, dim_theta, dim_hpars, lambda_model, X_init, COV_init, hpars_z_guess](string foldname, VectorXd const &Xobs, VectorXd const &Yobs)
  {
    ...
    {
      VectorXd x(1);
      x << Xobs(nobs - 1) * double(i) / double(samp_size);
      X_predictions[i] = x;
    }
    ...
    auto logprior_hpars = [](VectorXd const &hpar)
    {
    ...
    string fname_data = foldname + "obs" +to_string(Xobs.size()) + endname;
    ...
    double timeopt=0.5; //secondes par optimisation. devrait dépendre du nombre d'observations.
...
*/
    ...
    Dens.SetNewSamples(visited_steps);
    Dens.SetNewHparsOfSamples(visited_hpars);
    //Dens.WritePredictions(X_predictions,foldname+ "preds"+ to_string(Yobs.size()) +endname);
  ...
  auto extract_FD_grid = [&generator, draw_pts](VectorXd const &Xprev, VectorXd const &Yprev, int nobs)
  {
    ...
    if (nobs > Xprev.size())
    {
      cerr << "erreur ! taille grid trop grande" << endl;
    }
    if (nobs == Xprev.size())
    {
      cerr << "extracting same grid size ?" << endl;
    }
    VectorXd X(nobs);
    VectorXd Y(nobs);
    vector<int> indprev;
